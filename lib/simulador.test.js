...
test('planilha: o OEE entra uma vez, nos minutos por unidade — o disponível só multiplica', () => {
  ...
  assert.equal(jan[14].f, 'IF(N2=0,"",H2/N2)');
  ...
  assert.equal(ano[9].f,
    'IF(SUMPRODUCT(M2:M13,I2:I13)=0,"",SUMPRODUCT(J2:J13,M2:M13,I2:I13)/SUMPRODUCT(M2:M13,I2:I13))');
  assert.equal(ano[10].f,
    'IF(SUMPRODUCT(J2:J13,M2:M13,I2:I13)=0,"",N14/SUMPRODUCT(J2:J13,M2:M13,I2:I13))');
  assert.equal(ano[11].f, 'IF(SUMPRODUCT(M2:M13,I2:I13)=0,"",N14/SUMPRODUCT(M2:M13,I2:I13))');
  assert.equal(ano[12].f, 'IF(I14=0,"",SUMPRODUCT(M2:M13,I2:I13)/I14)');
...
test('planilha: a aba Por CC soma a Por CT com SUMIFS por planta, área, CC e mês', () => {
  ...
  assert.match(cc.linhas[0][6].f, /^SUMIFS\('Por CT'!\$M\$2:\$M\$27,/);   // unidades
  assert.match(cc.linhas[0][7].f, /^SUMIFS\('Por CT'!\$N\$2:\$N\$27,/);   // disponível
  assert.equal(cc.linhas[0][8].f, 'IF(H2=0,"",F2/H2)');
  const ano = cc.linhas[12];
  assert.equal(ano[4].v, 'Ano');
  assert.match(ano[7].f, /\$N\$2:\$N\$27.*"Ano"\)$/);
});
