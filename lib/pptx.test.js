...
import {
  ...
  preencheCampo, preencheSlide, retanguloDoCampo, slidesDo,
} from './pptx.js';
...
// --- a geometria do subtítulo ------------------------------------------------

const caixaComRotulo = (rotulo, x, y, cx, cy) =>
  '<p:sp><p:nvSpPr><p:cNvPr id="2" name="txt"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>'
  + `<p:spPr><a:xfrm><a:off x="${x}" y="${y}"/>`
  + `<a:ext cx="${cx}" cy="${cy}"/></a:xfrm></p:spPr>`
  + `<p:txBody><a:bodyPr/><a:p><a:r><a:t>${rotulo}</a:t></a:r></a:p></p:txBody>`
  + '</p:sp>';

const parte = (dentro) => `<?xml version="1.0"?><p:sld><p:cSld><p:spTree>${dentro}`
  + '</p:spTree></p:cSld></p:sld>';

test('o retângulo do subtítulo casa por RÓTULO, e não só por espaço reservado', () => {
  // Este é o defeito que passou em silêncio: o modelo em uso é montado à mão e
  // não tem <p:ph> nenhum — tem uma caixa onde alguém escreveu "Subtítulo".
  // Casando só por tipo, a função devolvia nulo, a pílula não subia e o desenho
  // não crescia, sem erro nenhum para denunciar.
  const arquivos = new Map([
    ['ppt/slides/slide1.xml',
     parte(caixaComRotulo('Subtítulo', 100, 200, 300, 400))],
  ]);

  assert.deepEqual(retanguloDoCampo(arquivos, 'ppt/slides/slide1.xml', 'subtitulo'),
                   { x: 100, y: 200, largura: 300, altura: 400 });
});

test('sem acento também casa, porque quem digitou pode ter deixado passar', () => {
  const arquivos = new Map([
    ['ppt/slides/slide1.xml',
     parte(caixaComRotulo('SUBTITULO', 1, 2, 3, 4))],
  ]);
  assert.ok(retanguloDoCampo(arquivos, 'ppt/slides/slide1.xml', 'subtitulo'));
});

test('não achando no slide, procura no leiaute para onde o slide aponta', () => {
  // Espaço reservado HERDA a posição: é por isso que arrastar o título num
  // slide não move o dos outros. Parar no slide devolveria nulo quase sempre.
  const arquivos = new Map([
    ['ppt/slides/slide1.xml', parte('')],
    ['ppt/slides/_rels/slide1.xml.rels',
     '<Relationships><Relationship Id="rId1" '
     + 'Target="../slideLayouts/slideLayout7.xml"/></Relationships>'],
    ['ppt/slideLayouts/slideLayout7.xml',
     parte('<p:sp><p:nvSpPr><p:cNvPr id="3" name="s"/><p:cNvSpPr/>'
       + '<p:nvPr><p:ph type="subTitle"/></p:nvPr></p:nvSpPr>'
       + '<p:spPr><a:xfrm><a:off x="9" y="8"/><a:ext cx="7" cy="6"/></a:xfrm>'
       + '</p:spPr><p:txBody><a:bodyPr/><a:p/></p:txBody></p:sp>')],
  ]);

  assert.deepEqual(retanguloDoCampo(arquivos, 'ppt/slides/slide1.xml', 'subtitulo'),
                   { x: 9, y: 8, largura: 7, altura: 6 });
});

test('sem geometria em lugar nenhum devolve nulo, e não um chute', () => {
  const arquivos = new Map([['ppt/slides/slide1.xml', parte('')]]);
  assert.equal(
    retanguloDoCampo(arquivos, 'ppt/slides/slide1.xml', 'subtitulo'), null);
});

// --- a quebra de linha do título ---------------------------------------------

test('o título em duas linhas vira <a:br/>, e não um caractere solto', () => {
  // Uma nova linha dentro de <a:t> vira ESPAÇO ao desenhar: o título sairia
  // numa linha só, sem erro nenhum para denunciar.
  const { xml, trocou } = preencheCampo(
    slide('Título'), 'titulo', 'S&OP\nCAPACITY TOOL');

  assert.ok(trocou);
  assert.ok(xml.includes('<a:br/>'), 'faltou a quebra');
  assert.ok(xml.includes('<a:t>S&amp;OP</a:t>'), 'o & tem que ser escapado');
  assert.ok(xml.includes('<a:t>CAPACITY TOOL</a:t>'));
  assert.ok(!/<a:t>[^<]*\n/.test(xml), 'nenhuma nova linha crua dentro de <a:t>');

  // As tags continuam fechando: run aberto e não fechado é arquivo que o
  // PowerPoint manda reparar, e reparar costuma significar perder o slide.
  for (const tag of ['a:r', 'a:t', 'a:p']) {
...
test('uma linha só sai exatamente como saía antes', () => {
  // A quebra é opcional e não pode mudar o caminho comum.
  const { xml } = preencheCampo(slide('Título'), 'titulo', 'Matriz - Tecelagem');
  assert.ok(xml.includes('<a:t>Matriz - Tecelagem</a:t>'));
  assert.ok(!xml.includes('<a:br/>'));
});

test('o par do Windows não deixa um CR solto no XML', () => {
  const { xml } = preencheCampo(slide('Título'), 'titulo', 'A\r\nB');
  assert.ok(xml.includes('<a:t>A</a:t>'));
  assert.ok(xml.includes('<a:t>B</a:t>'));
  // Sem contrabarra no teste: o CR e procurado pelo codigo do caractere,
  // que e o que de fato nao pode sobrar dentro do XML.
  assert.ok(!xml.includes(String.fromCharCode(13)), 'sobrou um CR');
});
