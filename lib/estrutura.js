 ...
 *
 * PESSOA NÃO TEM QUANTIDADE AQUI (migração 36). A Qtd do cadastro é o teto
 * físico da máquina — quantas existem —, e para pessoa não há teto: a
 * instalada dela é a planejada. Quantas pessoas trabalham é pergunta do TURNO,
 * onde muda de turno para turno. O 1 gravado existe só para a multiplicação
 * não zerar; a tela não pergunta, e o que vier no corpo é ignorado.
 */
async function garantirParametro(
  recursoId, { qt_recursos, equivalencia, inicio, fim, tipo_recurso },
) {
  const qt = tipo_recurso === 'PESSOA' ? 1 : Number(qt_recursos);
...
export async function criarRecurso({
  ...
  await garantirParametro(r[0].id, {
    qt_recursos, equivalencia, inicio, fim, tipo_recurso: tipo,
  });

  // OEE 100% DE NASCENÇA, nas duas origens e com vigência aberta dos dois
  // lados. Desde que a ausência de OEE vale 0% no motor, recurso sem cadastro
  // entraria zerado: apareceria na tela com disponível 0 sem que ninguém
  // tivesse decidido isso, e pareceria defeito.
  //
  // 100% não é um chute disfarçado de dado — é o "ainda não medi" dito em voz
  // alta, e ele aparece na tela de OEE para alguém corrigir. O que não pode
  // existir é recurso SEM linha nenhuma: aí o número vira zero calado.
  await sql`
    insert into recurso_oee (recurso_id, origem, turno_id, vigencia, oee_pct)
    select ${r[0].id}, o.origem, null, daterange(null, null), 1.0
 ...
 * e capacidade_instalada referenciam sem cascade, e apagar arrancaria a
...
export async function excluirRecurso(id) {
  ...
  // AS DUAS TABELAS, e não só o fato. Recurso sem turno — uma binadeira da
  // Fiação, por exemplo — não tem uma linha de fato sequer, mas tem TETO em
  // toda rodada que o viu: a instalada existe sem calendário e sem turno. Só
  // o fato era conferido, o delete chegava à chave estrangeira da instalada e
  // a tela mostrava o erro cru do banco. Entrar numa rodada é entrar em
  // qualquer uma das duas.
  const usos = await sql`
    select (select count(distinct execucao_id) from capacidade_fato
             where recurso_id = ${r})
         + (select count(distinct execucao_id) from capacidade_instalada
             where recurso_id = ${r}) as rodadas`;

  // Recurso que já entrou em rodada não some: fato e instalada apontam para
  // ele sem cascade, e apagar arrancaria a referência de números já divulgados.
  //
      ...
      update recurso_parametro
         set status_cadastro = false,
             ...
             vigencia = daterange(
                 ...
                 '[)')
       where recurso_id = ${r}`;
    return { desativado: true, motivo: `já entrou em rodada(s) de cálculo` };
  }
...
export async function recursosDesativados() {
  return sql`
    select r.id, r.codigo, r.nome,
           ...
           -- Fato E instalada: recurso sem turno so aparece na instalada, e
           -- "0 rodadas" ao lado dele diria que apagar nao mexe em nada.
           (select count(distinct x.execucao_id) from (
               select execucao_id from capacidade_fato      where recurso_id = r.id
               union
               select execucao_id from capacidade_instalada where recurso_id = r.id
            ) x)                                 as rodadas
      from recurso r
      join area a   on a.id = r.area_id
      join planta p on p.id = a.planta_id
 ...
 *
 * DESTRUTIVO e sem volta. capacidade_fato e capacidade_instalada apontam
...
export async function excluirRecursoDefinitivo(id) {
  ...
  const antes = await sql`
    select (select count(*) from capacidade_fato where recurso_id = ${r}) as fato,
           (select count(*) from capacidade_instalada where recurso_id = ${r}) as instalada,
  ...
  await sql.transaction([
    ...
    // Faixas, e não dias (migração 35): a contagem que volta para a tela é de
    // faixas — uma por rodada em que o recurso existia.
    sql`delete from capacidade_instalada    where recurso_id = ${r}`,
