...
const cartao = (id, { x, y, largura, altura }, {
  ...
  // O NOME APARECE NO PAINEL DE SELEÇÃO do PowerPoint, e é por ele que dá para
  // achar a forma certa para arrastar — e para ler de volta onde ela parou. Sem
  // nome, o painel lista "Retângulo 47" trinta vezes.
  nome = null,
} = {}) =>
  `<p:sp>${nv(id, nome ?? `c${id}`)}<p:spPr>${xfrm(x, y, largura, altura)}`
...
// Subiu de 0,52 quando o desenho passou a começar junto do subtítulo: a área
// ficou quase uma polegada mais alta, e 0,60 dela deixa a tabela com MAIS
// espaço do que os 0,52 da área baixa davam. A conta que importa é a altura da
// tabela em polegadas, não a fração.
const FATIA_GRAFICO = 0.60;
// A calha do rótulo das linhas. Subiu de 1,15 quando os rótulos passaram a
// levar a unidade e a faixa entre parênteses: "Ocupação (ociosidade)" precisa
// de 1,13 polegada a 7,5pt e tinha 0,95, então quebrava em duas linhas dentro
// de uma faixa de 0,17 — o texto saía por cima da linha de baixo.
const ROTULO_MAX = 1.38;
...
// ONDE A TABELA DE DADOS COMEÇA E TERMINA, em fração da largura do SLIDE — e
// não da caixa marcada. Saiu da régua do PowerPoint: o Bruno pediu de -13 cm a
// +16 cm, e a régua é centrada. Num slide de 33,87 cm o zero está em 16,93, de
// modo que -13 é 3,93 cm da borda esquerda e +16 é 32,93 — isto aqui.
//
// Medida do SLIDE de propósito. A caixa marcada diz onde há espaço livre, e é
// dela que sai o topo e a esquerda; mas a largura da tabela é decisão de quem
// projeta o slide na parede, e essa foi tomada com régua na mão.
const DADOS_DE = 3.93 / 33.87;
const DADOS_ATE = 32.93 / 33.87;

// ONDE OS CARTÕES DO GRUPO COMEÇAM, também em fração da largura do slide. Saiu
// medido do .pptx que o Bruno ajustou à mão: ele arrastou a borda ESQUERDA dos
// dois cartões de 0,667 para 0,297 polegada e não tocou na direita. A caixa
// marcada começa em 0,667, e é por isso que este número não pode sair dela — o
// modelo reservou dali para a frente, e o cartão do grupo mora antes disso.
const CARTAO_DE = 0.755 / 33.87;
...
/**
 * O MAIOR CORPO EM QUE O TEXTO CABE NUMA LINHA, em centésimos de ponto.
 *
 * Existe porque cartão estreito NÃO ENCOLHE TEXTO, ELE O DERRAMA: o PowerPoint
 * quebra a palavra e escreve por cima do que vier embaixo. Foi assim que
 * "CADASTROS" virou "CAD ASTR OS" com a descrição atravessada no meio, quando a
 * calha do grupo passou a ser medida pela régua e ficou menor que o cartão
 * suposto.
 *
 * Estimativa, e de propósito para MAIS: o DrawingML não mede texto, e errar
 * para o lado do corpo menor deixa o rótulo pequeno — errar para o outro
 * derrama de novo.
 */
function corpoQueCabe(texto, larguraTexto, maximo, minimo) {
  const porPonto = String(texto ?? '').length * pol(LARGURA_LETRA) / 100;
  if (porPonto <= 0 || larguraTexto <= 0) return maximo;
  const cabe = Math.floor(larguraTexto / porPonto / 25) * 25;
 ...
 */
/**
 * O TEXTO QUE CABE, cortado com reticências quando não cabe.
 *
 * A última rede depois de `corpoQueCabe`: o piso do corpo existe porque abaixo
 * de 5,5pt ninguém lê, e um nome de faixa de quarenta caracteres não cabe na
 * calha nem lá embaixo. Entre derramar por cima da linha seguinte e cortar, o
 * corte é o único que ainda deixa o slide legível — e as reticências dizem que
 * há mais, em vez de deixar parecer que o cadastro é aquilo.
 */
function cabeOuCorta(texto, largura, corpo) {
  const t = String(texto ?? '');
  const porLetra = pol(LARGURA_LETRA * corpo / 100);
  const cabem = porLetra > 0 ? Math.floor(largura / porLetra) : t.length;
  return t.length <= cabem ? t : `${t.slice(0, Math.max(1, cabem - 1))}…`;
}
...
export function areaDoVisual(retangulo, slide, {
  ...
  // A DIREITA VEM DA RÉGUA, e a esquerda da caixa marcada. São perguntas
  // diferentes: a caixa diz onde o modelo deixou espaço, a régua diz até onde o
  // Bruno quer que a tabela vá. Nunca encolhe abaixo da caixa — se a régua
  // pedisse menos que ela, o desenho é que estaria errado, não o modelo.
  const direita = Math.max(r.x + r.largura, slide.largura * DADOS_ATE);

  return {
    x: r.x,
    y: topo,
    largura: direita - r.x,
    // A CALHA DO GRUPO, medida e não estimada: é ela que faz a primeira coluna
    // da tabela cair exatamente onde a régua pediu. Zero quando a caixa marcada
    // já começa depois disso — aí não há calha para dar, e os cartões dos
    // blocos entram sobre a própria tabela em vez de à esquerda dela.
    calhaGrupo: Math.max(0, slide.largura * DADOS_DE - r.x),
    // A esquerda dos cartões do grupo, que fica ANTES da caixa marcada.
    cartaoGrupo: slide.largura * CARTAO_DE,
...
export function formasDoVisual({
  ...
  // A calha medida pela régua tem precedência sobre a estimada por proporção:
  // ela é o que faz a tabela começar onde foi pedido, e a proporção é só o
  // palpite de quem não recebeu medida nenhuma.
  const grupo = temGrupo
    ? (area.calhaGrupo ?? Math.min(pol(GRUPO_MAX), largura * 0.135)) : 0;
  ...
  if (visual.rodape) {
    ...
    saida.push(cartao(proximo(), caixaPilula,
      { fundo: 'accent1', adj: 50000, nome: 'CT pilula do periodo' }));
  ...
  // ---- o cartão do gráfico --------------------------------------------------
  // A LARGURA INTEIRA DA ÁREA. Ele já começou alinhado com a calha do grupo,
  // para a borda cair sobre a borda dos blocos — e o resultado foi um cartão
  // encolhido no meio de um slide vazio, com margem sobrando dos dois lados.
  //
  // O gráfico é a resposta da página; a tabela é a explicação. Quem manda no
  // tamanho é ele. O alinhamento que importa continua de pé, e não é o da
  // borda: é o das COLUNAS — a barra de março cai sobre março lá embaixo,
  // porque as duas usam a mesma calha.
  saida.push(cartao(proximo(),
    { x, y: cartaoY, largura, altura: cartaoH },
    { fundo: 'bg1', borda: 'tx1', alfaBorda: '15000', adj: 4000,
      nome: 'CT area do grafico' }));
  ...
  const itens = [
    // Bloco, e não tracinho: a capacidade virou área, e uma chave em forma de
    // linha representaria um traço que não existe mais no desenho. O tom é mais
    // forte que o da área — num quadrado de três milímetros, os 20% do gráfico
    // somem no branco e a legenda fica sem chave nenhuma.
    { esquema: 'accent1', texto: visual.rotuloCapacidade, alfa: '35000' },
  ...
  // ---- o gráfico ------------------------------------------------------------
  // A ÁREA PRIMEIRO e as barras depois: o DrawingML pinta na ordem do
  // documento, e área por cima cobriria a demanda com uma camada translúcida.
  //
  // SEM A LINHA NO TOPO DA ÁREA, por decisão do Bruno. Ela e a área diziam a
  // mesma coisa duas vezes — o contorno é a borda do preenchimento —, e o traço
  // grosso disputava atenção com as barras, que são a outra série. A área
  // sozinha delimita a capacidade e deixa a demanda ser lida.
  ...
  // O CORPO DO RÓTULO SAI DO MAIOR DELES, e vale para a coluna inteira.
  //
  // Uma faixa de ocupação pode ter até quarenta caracteres de nome cadastrado,
  // e "Ocupação (" + quarenta + ")" não cabe em calha nenhuma. Alargar a coluna
  // resolve o caso de hoje; isto resolve o de amanhã, e resolve do jeito que
  // não fica feio: todos os rótulos encolhem juntos, porque um menor que os
  // outros no meio da tabela parece defeito.
  //
  // Nunca MAIOR que 7,5pt — este é o tamanho do desenho, e a conta só serve
  // para descer dele quando for preciso.
  const larguraRotulo = rotulo - pol(0.20);
  const maiorRotulo = visual.linhas.reduce(
    (m, l) => (String(l.rotulo ?? '').length > m.length ? String(l.rotulo) : m),
    '');
  const corpoRotulo = corpoQueCabe(maiorRotulo, larguraRotulo, 750, 550);
  ...
  blocos.forEach((b, i) => {
    ...
    //
    // A CHAVE E A FOLGA ENCOLHEM COM A CALHA. Fixas em 0,12 e 0,26 polegada,
    // elas comiam 0,38 de uma calha de 0,85 e sobravam 0,25 para escrever
    // "CADASTROS" — que precisa de 0,58 no menor corpo legível. O que é
    // acessório cede espaço ao que é conteúdo, e não o contrário.
    const chaveW = Math.min(pol(CHAVE_LARGURA), grupo * 0.13);
    const folgaCartao = Math.min(pol(0.26), grupo * 0.13);

    // A DIREITA DO CARTÃO É QUEM MANDA — ela encosta na chave, e a chave na
    // tabela. A esquerda é que se estica, até onde o Bruno arrastou. Fazer o
    // contrário moveria a chave junto e desalinharia o abraço das linhas.
    const cartaoDireita = x + grupo - chaveW - folgaCartao;
    const cartaoX = Math.min(area.cartaoGrupo ?? x, cartaoDireita - pol(0.2));
    const cartaoW = cartaoDireita - cartaoX;
    const cartaoY2 = topo + pol(0.03);
    const cartaoH2 = base - topo - pol(0.06);
    saida.push(cartao(proximo(), {
      x: cartaoX, y: cartaoY2, largura: cartaoW, altura: cartaoH2,
    }, { fundo: tinta.esquema, alfaFundo: '14000', adj: 9000 }));

    // O recuo saiu medido do cartão que o Bruno ajustou: ele deixou 0,0655
    // polegada de cada lado num cartão de 1,021, e não os 0,09 proporcionais
    // que eu tinha posto. Meio milímetro não muda nada sozinho — mas o número
    // veio de quem olhou o slide projetado, e o meu veio de um palpite.
    const pad = Math.min(pol(0.11), cartaoW * 0.064);
    const larguraTexto = cartaoW - pad * 2;
    const nome = String(b.nome).toUpperCase();

    // O corpo sai da largura que sobrou, e não de um número escolhido de
    // antemão: é a única forma de o rótulo caber numa linha em qualquer calha.
    const corpoTitulo = corpoQueCabe(nome, larguraTexto, 900, 600);
    const linhasTitulo = Math.max(1, Math.ceil(
      nome.length * pol(LARGURA_LETRA * corpoTitulo / 100) / larguraTexto));
    // A ALTURA SEGUE O NÚMERO DE LINHAS. Fixa em 0,22 polegada, um título que
    // quebrasse em três escrevia por cima da descrição — que era exatamente o
    // que se via no slide.
    const tituloH = linhasTitulo * pol(0.145) + pol(0.05);

    saida.push(caixa(proximo(), {
      x: cartaoX + pad, y: cartaoY2 + pol(0.07),
      largura: larguraTexto, altura: tituloH,
    }, nome, {
      ...
      tamanho: corpoTitulo, negrito: true, alinhamento: 'ctr',
    ...
    // A margem de baixo era 0,14 e engolia a frase do bloco de três linhas: ela
    // precisa de 0,18 e sobravam 0,10. O Bruno pôs a frase lá à mão, o que é a
    // resposta de quem olhou — cabia, e a régua é que estava larga demais.
    const sobra = cartaoH2 - tituloH - pol(0.05);
    const corpo = 600;
    ...
    if (descricao && linhasDaFrase * alturaLinha <= sobra) {
      saida.push(caixa(proximo(), {
        x: cartaoX + pad, y: cartaoY2 + pol(0.07) + tituloH,
    ...
    saida.push(chaveDeGrupo(proximo(), {
      x: x + grupo - chaveW - Math.min(pol(0.08), grupo * 0.04),
      y: topo + pol(0.04),
      largura: chaveW,
  ...
  visual.linhas.forEach((linha, i) => {
    ...
    if (linha.rotulo) {
      saida.push(caixa(proximo(), {
        ...
        largura: larguraRotulo, altura: f.altura,
      }, cabeOuCorta(linha.rotulo, larguraRotulo, corpoRotulo), {
        tamanho: corpoRotulo, alinhamento: 'l', esquema: 'tx1', alfa: '80000',
      }));
    }
  ...
  // ---- a nota de rodapé -----------------------------------------------------
  // ABAIXO DA ÁREA, e não dentro dela: a grade termina exatamente onde a área
  // termina, e roubar altura dela mudaria a altura de todas as linhas — o
  // leiaute está fechado e não se mexe por causa de uma frase.
  //
  // O espaço existe: `MARGEM_INFERIOR` reserva 0,85 polegada para o rodapé do
  // modelo, e a faixa escura dele começa mais abaixo. A nota usa a fatia de
  // cima dessa reserva, alinhada à esquerda com a primeira coluna da tabela,
  // que é onde o olho volta depois de ler a última linha.
  //
  // Quando não há nota, nada é desenhado e o slide sai idêntico ao de antes.
  if (visual.nota) {
    saida.push(caixa(proximo(), {
      x: x + grupo, y: y + altura + pol(0.02),
      largura: largura - grupo, altura: pol(0.13),
    }, visual.nota, {
      tamanho: 700, alinhamento: 'l', esquema: 'tx1', alfa: '65000',
      ancora: 't',
    }));
  }
