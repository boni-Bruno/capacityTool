 ...
 */
const chaveDeGrupo = (id, { x, y, largura, altura }) =>
  ...
  + `<a:solidFill>${cor('accent1', '80000')}</a:solidFill><a:round/></a:ln>`
...
// A calha do grupo, à esquerda dos rótulos: o nome do bloco e a chave. Precisa
// caber "cadastros" em corpo 8 mais a chave, sem espremer as doze colunas.
const GRUPO_MAX = 1.0;
const CHAVE_LARGURA = 0.13;
...
export function formasDoVisual({ area, visual, idBase = 7000, fmt = String }) {
  ...
  const grupo = temGrupo ? Math.min(pol(GRUPO_MAX), largura * 0.095) : 0;
  ...
  const chaveW = pol(CHAVE_LARGURA);
  blocos.forEach((b, i) => {
    const topo = faixas[b.ini].y;
    const base = faixas[b.fim].y + faixas[b.fim].altura;

    // Uma régua na largura toda separando os blocos, mais forte que a linha
    // entre as faixas: é ela que diz onde um bloco acaba e o outro começa, e o
    // separador de linha comum não distingue "outra linha" de "outro assunto".
    if (i > 0) {
      saida.push(retangulo(proximo(), {
        x: x + grupo, y: topo - pol(0.008),
        largura: largura - grupo, altura: pol(0.016),
      }, 'tx1', '45000'));
    }

    // A chave encostada nos rótulos, e o nome à esquerda dela. A folga vertical
    // impede que a ponta da chave toque o número da primeira e da última linha.
    saida.push(chaveDeGrupo(proximo(), {
      x: x + grupo - chaveW - pol(0.07), y: topo + pol(0.03),
      largura: chaveW, altura: Math.max(pol(0.12), base - topo - pol(0.06)),
    }));
    saida.push(caixa(proximo(), {
      x, y: topo, largura: grupo - chaveW - pol(0.16), altura: base - topo,
    }, b.nome, {
      tamanho: 800, negrito: true, alinhamento: 'r', esquema: 'accent1',
    }));
  });
