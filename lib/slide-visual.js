...
export function formasDoVisual({ area, visual, idBase = 7000, fmt = String }) {
  ...
  const grupo = temGrupo ? Math.min(pol(GRUPO_MAX), largura * 0.145) : 0;
  ...
  const pilulaH = visual.rodape ? pol(0.30) : 0;
  ...
  if (visual.rodape) {
    ...
    const caixaPilula = { x: x + largura - w, y, largura: w, altura: pilulaH };
  ...
  blocos.forEach((b, i) => {
    ...
    const cartaoW = grupo - pol(CHAVE_LARGURA) - pol(0.24);
    saida.push(cartao(proximo(), {
      x, y: topo, largura: cartaoW, altura: base - topo,
    }, { fundo: tinta.esquema, alfaFundo: '14000', adj: 9000 }));

    const tituloH = pol(0.24);
    saida.push(caixa(proximo(), {
      x: x + pol(0.12), y: topo + pol(0.10),
      largura: cartaoW - pol(0.24), altura: tituloH,
    }, String(b.nome).toUpperCase(), {
      tamanho: 900, negrito: true, alinhamento: 'l',
    ...
    if (descricao) {
      saida.push(caixa(proximo(), {
        x: x + pol(0.12), y: topo + pol(0.10) + tituloH,
        largura: cartaoW - pol(0.24),
        altura: Math.max(pol(0.2), base - topo - tituloH - pol(0.20)),
      }, descricao, {
        tamanho: 700, alinhamento: 'l', esquema: 'tx1', alfa: '65000',
        ancora: 't', entrelinha: '90000',
      }));
    }
