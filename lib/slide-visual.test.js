...
// --- o rótulo da linha não derrama -------------------------------------------

const comRotulos = (rotulos, idBase) => formasDoVisual({
  area: areaDoVisual(
    { x: pol(0.667), y: pol(1.542), largura: pol(12.000), altura: pol(0.399) },
    SLIDE, { reservaTitulo: false }),
  idBase,
  visual: {
    ...
    linhas: [
      { rotulo: '', valores: ['jan', 'fev'], total: 'Ano', cabecalho: true },
      ...rotulos.map((r) => (
        { grupo: 'cálculo', rotulo: r, valores: ['1', '2'], total: '3' })),
    ],
  },
});

const caixaDoTexto = (xml, marca) => {
  const i = xml.indexOf(marca);
  assert.ok(i > -1, `não achei ${marca}`);
  const b = xml.slice(xml.lastIndexOf('<p:sp>', i), i);
  return {
    largura: Number(b.match(/<a:ext cx="(\d+)"/)[1]),
    corpo: Number(b.match(/sz="(\d+)"/)[1]),
  };
};

test('os rótulos com unidade e faixa cabem numa linha', () => {
  // O DEFEITO QUE ESTE TESTE EXISTE PARA NÃO TER: "Ocupação (ociosidade)" pede
  // 1,13 polegada a 7,5pt, e a calha do rótulo tinha 0,95 — o texto quebrava em
  // duas linhas dentro de uma faixa de 0,17 e saía por cima da linha de baixo.
  const rotulos = ['Capacidade (minutos)', 'Ocupação (ociosidade)',
                   'Paradas (minutos)', 'OEE (simulado)'];
  const x = comRotulos(rotulos, 1300);

  for (const r of rotulos) {
    const c = caixaDoTexto(x, `<a:t>${r}</a:t>`);
    const precisa = r.length * pol(0.0072 * c.corpo / 100);
    assert.ok(precisa <= c.largura,
      `"${r}" a ${c.corpo / 100}pt precisa de ${precisa} e tem ${c.largura}`);
  }
});

test('todos os rótulos saem no MESMO corpo, e nunca maior que 7,5pt', () => {
  // Um rótulo menor que os outros no meio da tabela parece defeito. E o corpo
  // só desce do padrão: a conta serve para caber, não para crescer.
  const x = comRotulos(['Capacidade (minutos)', 'OEE (meta)'], 1400);
  const corpos = [...x.matchAll(/sz="(\d+)"/g)].map((m) => Number(m[1]));
  assert.ok(corpos.includes(750), 'o padrão continua 7,5pt');
  assert.ok(Math.max(...corpos.filter((c) => c <= 750)) <= 750);
});

test('nome de faixa comprido demais é cortado, e não derramado', () => {
  // Uma faixa aceita quarenta caracteres de nome no cadastro, e nem no piso de
  // 5,5pt isso cabe na calha. Cortar deixa o slide legível; derramar escreve
  // por cima da linha seguinte.
  const enorme = 'Ocupação (abaixo do mínimo aceitável pela diretoria)';
  const x = comRotulos([enorme, 'Capacidade (minutos)'], 1500);

  assert.ok(!x.includes(`<a:t>${enorme}</a:t>`), 'não pode sair inteiro');
  const c = caixaDoTexto(x, '<a:t>Ocupação (abaixo');
  const escrito = x.slice(x.indexOf('<a:t>Ocupação (abaixo') + 5)
    .slice(0, x.slice(x.indexOf('<a:t>Ocupação (abaixo') + 5).indexOf('</a:t>'));
  assert.ok(escrito.endsWith('…'), `deveria terminar em reticências: "${escrito}"`);
  assert.ok(escrito.length * pol(0.0072 * c.corpo / 100) <= c.largura);
});
