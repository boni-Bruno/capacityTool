...
test('a faixa cadastrada pinta a célula, e só ela sai fora do tema', () => {
  // A cor da ocupação é a única exceção: ela não decora, informa, e quem
  // escolheu foi quem conhece a régua da fábrica.
  ...
  // Texto branco sobre o verde escuro e preto sobre o âmbar: célula ilegível
  // num slide projetado é pior que célula sem cor.
  assert.ok(x.includes('srgbClr val="FFFFFF"'));
  assert.ok(x.includes('srgbClr val="000000"'));
});
