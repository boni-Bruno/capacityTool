...
//
// O lote precisa desta leitura para MESCLAR em vez de substituir: aplicar 78%
// em janeiro a nove CTs não pode apagar o que eles têm no resto do ano.
...
test('o mês preenchido do lote vence, e os outros ficam como estavam', () => {
  // É a conta que o lote faz: ler o ano, sobrepor só o que foi digitado.
  const atuais = mesesDoAno(
    [{ inicio: '2026-01-01', fim: null, valor: '0.85000' }], 2026);
  const mesclado = { ...atuais, 1: '0.78000' };
  assert.equal(mesclado[1], '0.78000');
  assert.equal(mesclado[2], '0.85000');
  assert.equal(Object.keys(mesclado).length, 12);
});
