...
// montado em pedaços.
export async function totais(execucaoId, areaId, ano, recursoId = null) {
  const r = await sql`
    select
      (select coalesce(sum(min_instalada), 0)
        ...
        where execucao_id = ${execucaoId}
          ...
          and (${recursoId}::int is null
               or recurso_id = ${recursoId}::int))             as min_instalada,
      (select coalesce(sum(min_planejada), 0)
        ...
        where execucao_id = ${execucaoId}
          ...
          and extract(year from data) = ${ano}
          and (${recursoId}::int is null
               or recurso_id = ${recursoId}::int))             as min_planejada,
      (select coalesce(sum(min_disponivel), 0)
        ...
        where execucao_id = ${execucaoId}
          ...
          and extract(year from data) = ${ano}
          and (${recursoId}::int is null
               or recurso_id = ${recursoId}::int))             as min_disponivel`;
  return r[0];
}
