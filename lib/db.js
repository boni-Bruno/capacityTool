...
// cada par área×ano entra com a ÚLTIMA rodada OK da origem escolhida (META ou
// SIMULADO) — a mesma regra do painel, aplicada em lote. Misturar rodadas de idades diferentes é
    ...
    return sql`
      with rodada as (
        select i.area_id, extract(year from i.data)::int as ano,
         ...
         where e.status = 'OK' and e.origem = ${og}
         group by 1, 2
      )
