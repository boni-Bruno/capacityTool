 ...
 * doze meses para o mês vazio não sumir. O que ela tem de próprio é
 * `por_unidade`: a soma de min_disponivel ÷ qt_recursos linha a linha do fato.
 * Cada linha do fato é um turno de um dia, com a quantidade DAQUELE turno, e é
 * por isso que a divisão tem que ser antes da soma: dividir a soma do mês pela
 * quantidade cadastrada erraria em todo recurso que roda 3 máquinas de dia e
 * 1 de noite. `nullif` protege o recurso cadastrado com zero — ele entra com
 * capacidade zero, não com erro.
 *
 * Pessoa entra igual: para ela `qt_recursos` do fato é a quantidade de pessoas
 * do turno (migração 36), e a planejada é linear nisso do mesmo jeito. A
 * coluna `unidade` só diz à planilha se o número é de máquinas, de pessoas ou
 * dos dois misturados no mesmo CT — caso em que somar não faz sentido e o
 * rótulo avisa.
 *
 * O ano inteiro sempre, e não um intervalo: o simulador dimensiona pelo ano e
 * pelo pico, e um pico dentro de um recorte de três meses seria um pico falso.
 */
export async function simuladorPorCtMes(areaIds, ccs, ano, origem = 'META',
  ...
  return sql`
    ...
    cap as (
      select a.chave, extract(month from f.data)::int as mes,
             sum(f.min_disponivel)::float8 as disponivel,
             sum(f.min_disponivel / nullif(f.qt_recursos, 0))::float8
               as por_unidade
        from capacidade_fato f
        join rodada x on x.id = f.execucao_id
        join alvo a   on a.id = f.recurso_id
    ...
    select i.chave as ct, i.planta, i.area, i.cc, i.recursos, i.unidade,
           ...
           coalesce(dem.demanda, 0)     as demanda,
           coalesce(cap.disponivel, 0)  as disponivel,
           coalesce(cap.por_unidade, 0) as por_unidade
      from ident i
      cross join mes
      left join cap on cap.chave = i.chave and cap.mes = mes.mes
      left join dem on dem.chave = i.chave and dem.mes = mes.mes
