 ...
 *
 * OS MINUTOS DE PARADA saem de `capacidade_fato.min_parada_planejada`, que é o
 * que o motor DESCONTOU nesta mesma rodada — e não uma soma da tabela `parada`.
 * Somar o cadastro daria outro número: parada vale por turno, o intervalo dela
 * pode cair fora do calendário do recurso, e "dia inteiro" não é uma quantidade
 * de minutos, é um dia zerado. O slide mostraria um desconto que a barra ao
 * lado não teve.
 */
  ...
  return sql`
    ...
    cap as (
      select a.chave, extract(month from f.data)::int as mes,
             sum(f.min_planejada)::float8         as planejada,
             sum(f.min_disponivel)::float8        as disponivel,
             sum(f.min_parada_planejada)::float8  as parada
        from capacidade_fato f
        join rodada x on x.id = f.execucao_id
        join alvo a   on a.id = f.recurso_id
       where f.data between ${de}::date and ${ate}::date
...
export async function turnosDoRecorte(areaIds, ccs, ano, de, ate) {
  ...
  return sql`
    ...
    select a.chave as ct, min(a.planta) as planta, min(a.area) as area,
           min(a.cc) as cc, t.id as turno_id, t.nome as turno, mes.mes,
           sum(coalesce(rt.qt_recursos, a.qt))::int as qt
      from alvo a
     ...
     where rt.vigencia && daterange(
             ...
             '[)')
     group by a.chave, t.id, t.nome, mes.mes
     order by 1, 6, 7`;
}
