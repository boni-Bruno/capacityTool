...
/**
 * O cadastro do recorte, em números.
 *
 * Contagens e não listas: um slide não comporta duzentos recursos, e a pergunta
 * que ele responde é "como está configurado", não "quem são todos eles". O
 * detalhe fica na página de impressão, que tem quantas páginas precisar.
 */
export async function configuracaoDoRecorte(areaIds, ccs) {
  const areas = (areaIds ?? []).join(',') || '0';
  const lista = (ccs ?? []).join(',') || null;

  return sql`
    with alvo as (
      select r.id, r.tipo_recurso, m.cc, m.ct, r.nome, r.area_id,
       ...
       where r.area_id = any(string_to_array(${areas}, ',')::int[])
         and (${lista}::text is null
              or m.cc = any(string_to_array(${lista}, ',')))
    )
    select (select count(*) from alvo)::int                          as recursos,
           (select count(*) from alvo where tipo_recurso = 'MAQUINA')::int
             as maquinas,
           (select count(*) from alvo where tipo_recurso = 'PESSOA')::int
             as pessoas,
           (select coalesce(sum(qt), 0) from alvo)::int              as postos,
           (select count(distinct cc) from alvo)::int                as ccs,
           (select count(distinct cc || '-' || ct) from alvo)::int    as cts,
           (select count(distinct t.turno_id)
              from recurso_turno t where t.recurso_id in (select id from alvo))::int
             as turnos,
           (select count(distinct c.calendario_id)
              from recurso_calendario c
             where c.recurso_id in (select id from alvo))::int        as calendarios,
           (select count(*) from recurso_oee o
             where o.recurso_id in (select id from alvo))::int        as faixas_oee,
           (select count(*) from parada p
             where p.recurso_id in (select id from alvo))::int        as paradas`;
}

/**
 * A capacidade do recorte no ano, por medida.
 *
 * Lê a mesma rodada que o painel lê — uma por área, ano e origem desde a
 * migração 26 —, então o documento e a tela nunca contam números diferentes.
 */
export async function capacidadeDoRecorte(areaIds, ccs, ano, origem = 'META') {
  const areas = (areaIds ?? []).join(',') || '0';
  const lista = (ccs ?? []).join(',') || null;

  return sql`
    with rodada as (
      select id, area_id from calculo_execucao
    ...
    select coalesce((select sum(c.min_instalada)
                       from capacidade_instalada_dia c
                       join rodada x on x.id = c.execucao_id
                      where c.recurso_id in (select id from alvo)), 0)::float8
             as instalada,
           coalesce((select sum(f.min_planejada)
                       from capacidade_fato f
                       join rodada x on x.id = f.execucao_id
                      where f.recurso_id in (select id from alvo)), 0)::float8
             as planejada,
           coalesce((select sum(f.min_disponivel)
                       from capacidade_fato f
                       join rodada x on x.id = f.execucao_id
                      where f.recurso_id in (select id from alvo)), 0)::float8
             as disponivel,
           (select count(*) from rodada)::int as rodadas`;
}
...
export async function salvarModeloSlide({ arquivo, base64, slideMarca, slides }) {
  ...
  await sql`
    ...
    values (true, ${nome}, decode(${base64}, 'base64'), ${slideMarca ?? null},
            ${Number(slides) || 0}, ${Math.round((base64.length * 3) / 4)})
    on conflict (id) do update
       set arquivo = excluded.arquivo, conteudo = excluded.conteudo,
           slide_marca = excluded.slide_marca, slides = excluded.slides,
           tamanho = excluded.tamanho, criado_em = now()`;
}
