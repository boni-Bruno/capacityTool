...
// UMA RODADA OU VÁRIAS, UMA ÁREA OU VÁRIAS.
//
// As consultas do painel recebem `execucaoId` e `areaId` como um id ou como
// "id1,id2,…" — é assim que "todas as fábricas" entra: a rodada continua sendo
// por área, e o painel de todas soma as rodadas de cada uma. A lista vai como
// texto e vira array no SQL, o mesmo caminho que o filtro de recursos já usa.
// Vazio vira '0', que não casa com ninguém, em vez de estourar no cast.
const lst = (v) => String(v ?? '').split(',').map((s) => s.trim())
  .filter(Boolean).join(',') || '0';

/**
 * As rodadas que valem para VÁRIAS áreas num ano e origem — uma por área, a
 * mais recente de cada. É o que o painel de todas as fábricas soma.
 */
export async function rodadasDasAreas(areaIds, ano, origem = 'META') {
  return sql`
    select distinct on (e.area_id)
           e.id, e.area_id, e.periodo_inicio, e.periodo_fim, e.concluido_em,
           e.origem, e.parcial_em, c.nome as cenario
      from calculo_execucao e
      join cenario c on c.id = e.cenario_id
     where e.status = 'OK'
       and e.origem  = ${origem}
       and e.area_id = any(string_to_array(${lst(areaIds)}, ',')::int[])
       and extract(year from e.periodo_inicio) = ${ano}
     order by e.area_id, e.id desc`;
}
...
export async function ultimaExecucao(areaId, ano, origem = 'META') {
  const r = await sql`
    select e.id, e.periodo_inicio, e.periodo_fim, e.concluido_em, e.origem,
           e.parcial_em, c.nome as cenario
      from calculo_execucao e
      join cenario c on c.id = e.cenario_id
     where e.status = 'OK'
       and e.origem  = ${origem}
...
// `cache` do React: a MESMA lista por requisição, por mais gente que peça.
//
// Ela é pedida três vezes numa tela: a sessão (para resolver o escopo em
// áreas), o seletor de fábrica e, nas telas de estrutura, o recorte das
// listas. Eram três round trips iguais ao Neon; viraram um. O cache é por
// requisição — dado novo continua aparecendo na navegação seguinte, e o
// cookie de ordenação que `ordenarComoNaTela` lê é o mesmo dentro dela.
export const areas = cache(async () => {
  const linhas = await sql`
    select a.id, a.codigo, a.nome, a.planta_id, p.nome as planta,
...
// A INSTALADA VEM DE UMA VIEW, não de uma tabela de dias.
//
// `vw_instalada_dia` tem a forma recurso × dia que estas consultas sempre
// leram, mas é gerada na hora a partir de `capacidade_instalada`, que guarda
// uma FAIXA por recurso e rodada: minutos por dia e a vigência em que valem.
// Guardar 365 linhas com o mesmo número era o que estourava o Neon — ver a
// migração 35. Para pessoa, o teto é a planejada, e a view a lê do fato.
//
// Nenhuma consulta mudou de forma: elas continuam somando `min_instalada` por
// `data`, e o dia a dia do drill-down sai exato — o valor de um dia nunca
// dependeu de outro dia.
  ...
  return sql`
    ...
    inst as (
      select extract(month from c.data)::int as mes,
       ...
       where c.execucao_id = any(string_to_array(${lst(execucaoId)}, ',')::bigint[]) and c.area_id = any(string_to_array(${lst(areaId)}, ',')::int[])
    ...
    fato as (
      select extract(month from f.data)::int as mes,
       ...
       where f.execucao_id = any(string_to_array(${lst(execucaoId)}, ',')::bigint[]) and f.area_id = any(string_to_array(${lst(areaId)}, ',')::int[])
 ...
 *
 * Sem instalada — ela é grão dia (vw_instalada_dia), não grão turno.
...
export async function porTurnoDoDia(execucaoId, areaId, data, recursos = null,
  ...
  return sql`
    ...
    select t.codigo,
      ...
      left join fa on fa.ct  = mf.cc || '-' || mf.ct
                  and fa.mes = date_trunc('month', f.data)::date
     where f.execucao_id = any(string_to_array(${lst(execucaoId)}, ',')::bigint[])
       and f.area_id     = any(string_to_array(${lst(areaId)}, ',')::int[])
...
export async function tetoDoDia(execucaoId, areaId, data, recursos = null,
  ...
  const r = await sql`
    ...
    select coalesce(sum(c.min_instalada * coalesce(fa.fatia, 1)), 0)::numeric
      ...
      left join fa on fa.ct  = mf.cc || '-' || mf.ct
                  and fa.mes = date_trunc('month', c.data)::date
     where c.execucao_id = any(string_to_array(${lst(execucaoId)}, ',')::bigint[])
       and c.area_id     = any(string_to_array(${lst(areaId)}, ',')::int[])
...
// A leitura do memorial (memoriaDoDia) morou aqui até 11/09/2026 e saiu junto
// com o bloco do painel que a mostrava: o motor não grava o memorial desde a
// migração 33. Está no histórico do git, com o componente app/painel/memoria.jsx.
    ...
    select r.id,
      ...
      left join lateral (
            select sum(c.min_instalada * coalesce(fa.fatia, 1)) as m
              ...
              left join fa on fa.ct  = mf.cc || '-' || mf.ct
                          and fa.mes = date_trunc('month', c.data)::date
             where c.recurso_id = r.id and c.execucao_id = any(string_to_array(${lst(execucaoId)}, ',')::bigint[])
      ...
      left join lateral (
            ...
            select sum(cf.min_planejada  * coalesce(fr.fatia, 1)) as p,
             ...
             where cf.recurso_id = r.id and cf.execucao_id = any(string_to_array(${lst(execucaoId)}, ',')::bigint[])
               and cf.data between ${de}::date and ${ate}::date) f on true
     where r.area_id = any(string_to_array(${lst(areaId)}, ',')::int[])
...
/**
 * A MESMA leitura de porRecurso, aberta por MÊS — a matéria-prima da tabela
 * dinâmica do painel da capacidade.
 *
 * Uma linha por recurso e mês com os mesmos campos de identidade (planta, área,
 * sub-área, CC, CT, código, nome, tipo, calendário) e as mesmas medidas, em
 * minuto e convertidas. O mês é o grão que faz "média" significar alguma coisa:
 * média mensal de um CC é uma pergunta; média por recurso no período inteiro é
 * outra, e a tabela dinâmica deixa a pessoa escolher a ordem dos níveis.
 *
 * A fatia e o índice entram exatamente como em porRecurso — por CT e mês —,
 * então somar os doze meses de um recurso aqui dá o que porRecurso dá para o
 * mesmo recurso no mesmo período. É essa igualdade que faz a tabela dinâmica
 * fechar com o resto do painel.
 *
 * O filtro de recursos fica com a tela: ela já sabe quais ids passaram no funil
 * e recorta as linhas em JavaScript, como faz com a tabela por recurso.
 */
export async function porRecursoMes(execucaoId, areaId, de, ate, cargaId = null,
                                    fa = SEM_FATIA) {
  return sql`
    with fa as (
      select * from unnest(${fa.cts}::text[], ${fa.meses}::date[],
        ...
        as t(ct, mes, fatia, metros_por_min, qtd_por_min)),
    cap as (
      select cf.recurso_id,
             date_trunc('month', cf.data)::date as mes,
             sum(cf.min_planejada  * coalesce(fr.fatia, 1)) as p,
             sum(cf.min_disponivel * coalesce(fr.fatia, 1)) as d,
             sum(cf.min_planejada  * coalesce(fr.fatia, 1)
                 * coalesce(fr.metros_por_min, idx.metros_por_min, 0)) as pm,
             sum(cf.min_disponivel * coalesce(fr.fatia, 1)
             ...
             sum(cf.min_disponivel * coalesce(fr.fatia, 1)
                 * coalesce(fr.qtd_por_min, idx.qtd_por_min, 0))       as du
        from capacidade_fato cf
        join recurso r         on r.id = cf.recurso_id
        join maquina_fisica mf on mf.id = r.maquina_fisica_id
        left join mv_demanda_indice idx
               on idx.carga_id = ${cargaId}::int
              and idx.ct       = mf.cc || '-' || mf.ct
        ...
        left join fa fr
               on fr.ct  = mf.cc || '-' || mf.ct
              and fr.mes = date_trunc('month', cf.data)::date
       where cf.execucao_id = any(string_to_array(${lst(execucaoId)}, ',')::bigint[]) and cf.area_id = any(string_to_array(${lst(areaId)}, ',')::int[])
         and cf.data between ${de}::date and ${ate}::date
       group by 1, 2
    ),
    inst as (
      select c.recurso_id,
    ...
    meses as (
      select recurso_id, mes from cap
      union
      select recurso_id, mes from inst
    )
    select r.id,
           r.codigo,
           r.nome,
           ...
           cal.codigo                    as calendario,
           m.mes::text                   as mes,
           coalesce(inst.m, 0)::float8   as instalada,
           coalesce(cap.p, 0)::float8    as planejada,
           coalesce(cap.d, 0)::float8    as disponivel,
           coalesce(cap.pm, 0)::float8   as planejada_m,
           coalesce(cap.dm, 0)::float8   as disponivel_m,
           coalesce(cap.pu, 0)::float8   as planejada_u,
           coalesce(cap.du, 0)::float8   as disponivel_u
      from meses m
      join recurso r         on r.id = m.recurso_id
      join maquina_fisica mf on mf.id = r.maquina_fisica_id
      join area ar   on ar.id = r.area_id
      join planta pl on pl.id = ar.planta_id
      left join recurso_calendario rc
            ...
            and rc.vigencia @> ${ate}::date
      left join calendario cal on cal.id = rc.calendario_id
      left join cap  on cap.recurso_id  = m.recurso_id and cap.mes  = m.mes
      left join inst on inst.recurso_id = m.recurso_id and inst.mes = m.mes
     where (${fa.ativo}::boolean is not true
            or exists (select 1 from fa where fa.ct = mf.cc || '-' || mf.ct))
     order by r.nome, m.mes`;
}
 ...
 *
 * COM `recursos`, É PARCIAL (migração 37): o motor entra na rodada que existe
 * e regrava só aqueles recursos — nenhuma rodada nova, e a rodada ganha a
 * marca `parcial_em`, que o rodapé do painel mostra. Recurso não tem rodada
 * própria, e uma rodada nova só com três máquinas apagaria o resto da área.
 * Sem rodada para (área, ano, origem) não há o que atualizar: cai na rodada
 * cheia, que custa o mesmo.
 */
export async function recalcular(areaId, ano, origem = 'META', recursos = null) {
  const ids = (recursos ?? []).map(Number).filter((x) => Number.isInteger(x) && x > 0);

  if (ids.length) {
    const alvo = await sql`
      select id from calculo_execucao
       where status = 'OK' and origem = ${origem} and area_id = ${areaId}
         and periodo_inicio = make_date(${ano}, 1, 1)
         and periodo_fim    = make_date(${ano}, 12, 31)
    ...
    if (alvo.length) {
      ...
      const c = await sql`
        select (select count(*) from capacidade_instalada
                 where execucao_id = ${id} and recurso_id = any(${ids}::int[]))
                 as instalada,
               (select count(*) from capacidade_fato
                 where execucao_id = ${id} and recurso_id = any(${ids}::int[]))
                 as fato`;
      return {
        id, parcial: true,
        instalada: Number(c[0].instalada), fato: Number(c[0].fato),
      };
    }
  }
  ...
  // A instalada e contada em FAIXAS, nao em dias (migracao 35): uma por
  // recurso que existia no periodo. O que a tela quer saber e se a rodada
  // gerou alguma coisa, e para isso a faixa responde igual ao dia.
  const c = await sql`
    select (select count(*) from capacidade_instalada where execucao_id = ${id})
             as instalada,
 ...
 *
 * O Recalcular parcial usa a mesma lista, pelo mesmo funil: `area_id` está
 * aqui porque é por área que o motor roda, e a tela precisa agrupar.
 */
export async function recursosParaExtracao() {
  return sql`
    select r.id, r.codigo, r.nome, coalesce(r.sub_area, '') as sub_area,
           ...
           p.nome as planta, a.nome as area, a.id as area_id
      from recurso r
      join maquina_fisica mf on mf.id = r.maquina_fisica_id
...
/**
 * A MESMA leitura de ocupacaoPorCt, aberta por MÊS — a matéria-prima da tabela
 * dinâmica do painel da ocupação.
 *
 * Uma linha por CT e mês, com o mesmo FULL JOIN entre capacidade e demanda: o
 * CT que só tem demanda num mês aparece naquele mês, e é o caso que mais
 * importa. Somar os meses de um CT aqui dá o que ocupacaoPorCt dá para ele no
 * período — a tabela dinâmica precisa fechar com a tabela de cima.
 */
export async function ocupacaoPorCtMes(execucaoId, areaId, de, ate,
                                       recursos = null, cargaId = null) {
  return sql`
    with ident as (
      -- A identidade do CT vem do cadastro, e nao da linha de capacidade do
      -- mes: um CT com demanda em marco e capacidade so em abril precisa ter a
      -- mesma planta nos dois meses, senao a tabela dinamica o parte em dois.
      select mf.cc || '-' || mf.ct                as ct,
             min(mf.cc)                           as cc,
             string_agg(distinct r.nome, ', ')    as recursos,
             min(pl.nome)                         as planta,
    ...
    -- A instalada entra na uniao das chaves, e nao so por left join: um
    -- recurso sem turno num mes tem teto e nao tem fato, e sem esta linha a
    -- soma dos meses da tabela dinamica ficaria abaixo da instalada do painel.
    chaves as (
      select ct, mes from cap
      union select ct, mes from dem
      union select ct, mes from inst
    )
    select k.ct,
           coalesce(i.cc, split_part(k.ct, '-', 1))       as cc,
           coalesce(i.recursos, '')                       as recursos,
           coalesce(i.planta, '')                         as planta,
           coalesce(i.area, '')                           as area,
           k.mes::text                                    as mes,
           coalesce(inst.instalada, 0)                    as instalada,
           coalesce(cap.planejada, 0)                     as planejada,
           coalesce(cap.disponivel, 0)                    as disponivel,
           coalesce(dem.demanda, 0)                       as demanda
      from chaves k
      left join cap   on cap.ct  = k.ct and cap.mes  = k.mes
      left join dem   on dem.ct  = k.ct and dem.mes  = k.mes
      left join inst  on inst.ct = k.ct and inst.mes = k.mes
      left join ident i on i.ct  = k.ct
     order by 1, 6`;
}
...
/**
 * O RECORTE PARA O SIMULADOR DE QUANTIDADE DE RECURSOS: por CT e mês, o que
 * a demanda pede, o que a rodada entrega e o que UMA unidade entrega.
 *
 * Irmã de `serieDoRecorte` — mesma rodada, mesmo alvo, mesmo cross join com os
 * doze meses para o mês vazio não sumir. O que ela tem de próprio são os dois
 * fatores que a planilha decompõe do disponível:
 *
 *   dias_uteis     dias do mês em que o motor achou dia útil para algum
 *                  recurso do CT — o calendário do motor, sem os dias úteis
 *                  de apresentação;
 *   unidade_dias   Σ qt_recursos das linhas de dia útil. Cada linha do fato é
 *                  um turno de um dia com a quantidade DAQUELE turno, então
 *                  isso é "pessoas-dia" (ou máquinas-dia) somando os turnos:
 *                  10 no 1º e 8 no 2º dão 18 por dia útil, que é como o
 *                  Bruno cadastra e como ele quer ler.
 *
 * A planilha tira daí unidades por dia (unidade_dias ÷ dias_uteis) e minutos
 * por unidade por dia (planejada ÷ unidade_dias), e reconstrói o disponível
 * por fórmula: unidades × min/dia × dias úteis × OEE. Com os valores da rodada
 * a fórmula devolve exatamente o disponível do painel — é a prova de que ela
 * está certa — e com outra quantidade responde a pergunta.
 *
 * A primeira versão (11/09) dividia min_disponivel por qt_recursos linha a
 * linha e chamava de "por unidade" o que UMA pessoa entregava somando os
 * turnos: 9 "atuais" para 10 + 8 cadastradas. Aritmética certa, conceito
 * errado — ninguém cadastra pessoa de dois turnos.
 *
 * O ano inteiro sempre, e não um intervalo: o simulador é por mês com linha de
 * ano, e o ano só fecha com os doze.
 */
export async function simuladorPorCtMes(areaIds, ccs, ano, origem = 'META',
                                        cargaId = null) {
  const areas = (areaIds ?? []).join(',') || '0';
  const lista = (ccs ?? []).join(',') || null;
  const de = `${Number(ano)}-01-01`;
  const ate = `${Number(ano)}-12-31`;
  ...
  return sql`
    ...
    ident as (
      select chave, min(planta) as planta, min(area) as area, min(cc) as cc,
             string_agg(distinct nome, ', ') as recursos,
             case when count(distinct tipo_recurso) > 1 then 'MISTO'
                  else min(tipo_recurso) end as unidade
        from alvo group by 1
    ),
    mes as (select generate_series(1, 12) as mes),
    cap as (
      select a.chave, extract(month from f.data)::int as mes,
             sum(f.min_planejada)::float8  as planejada,
             sum(f.min_disponivel)::float8 as disponivel,
             count(distinct f.data) filter (where f.dia_util)::int as dias_uteis,
             coalesce(sum(f.qt_recursos) filter (where f.dia_util), 0)::float8
               as unidade_dias
        from capacidade_fato f
        join rodada x on x.id = f.execucao_id
        join alvo a   on a.id = f.recurso_id
    ...
    select i.chave as ct, i.planta, i.area, i.cc, i.recursos, i.unidade,
           mes.mes,
           coalesce(dem.demanda, 0)       as demanda,
           coalesce(cap.planejada, 0)     as planejada,
           coalesce(cap.disponivel, 0)    as disponivel,
           coalesce(cap.dias_uteis, 0)    as dias_uteis,
           coalesce(cap.unidade_dias, 0)  as unidade_dias
      from ident i
      cross join mes
      left join cap on cap.chave = i.chave and cap.mes = mes.mes
      left join dem on dem.chave = i.chave and dem.mes = mes.mes
