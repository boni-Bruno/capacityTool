  ...
  return sql`
    with inst as (
      select data, sum(min_instalada) as m
       ...
       group by 1),
    fato as (
      select data, sum(min_planejada) as p, sum(min_disponivel) as d
        from capacidade_fato
       where execucao_id = ${execucaoId} and area_id = ${areaId}
         and data between ${de}::date and ${ate}::date
         and (${recursos}::text is null
...
export async function porRecurso(execucaoId, areaId, de, ate) {
  return sql`
    select r.id,
           r.codigo,
           r.nome,
      ...
      left join lateral (
            select sum(min_planejada) as p, sum(min_disponivel) as d
              from capacidade_fato
             where recurso_id = r.id and execucao_id = ${execucaoId}
               and data between ${de}::date and ${ate}::date) f on true
