...
import {
  corDaOcupacao, corDoTexto, faixaDe, normalizaCor, rotuloFaixa, validaFaixas,
} from './faixa-cor.js';
...
test('o texto muda de preto para branco conforme o fundo', () => {
  // Texto escuro sobre vermelho escuro é célula ilegível — e ilegível num slide
  // projetado é pior que sem cor nenhuma.
  assert.equal(corDoTexto('#C62828'), '#FFFFFF');
  assert.equal(corDoTexto('#F9A825'), '#000000');
  assert.equal(corDoTexto('#FFFFFF'), '#000000');
  assert.equal(corDoTexto('#000000'), '#FFFFFF');
  assert.equal(corDoTexto('nada'), null);
});
