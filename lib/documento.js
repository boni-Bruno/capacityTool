...
import { corDaOcupacao, faixaDe } from './faixa-cor.js';
...
export const GRANULARIDADES = [
  ...
  { valor: 'ABERTO', rotulo: 'Aberto por CT + consolidações',
    dica: 'planta, área e, para cada CC, o consolidado dele seguido dos CTs' },
];

// Os níveis que o ABERTO percorre, do maior para o menor. RESUMO fica de fora:
// com uma planta e uma área no recorte ele repetiria os mesmos números um
// terceiro slide seguido, e três slides idênticos em sequência fazem quem
// assiste achar que a apresentação travou.
const NIVEIS_ABERTO = ['PLANTA', 'AREA'];
...
/**
 * O TÍTULO DA CAPA, em duas linhas.
 *
 * O modelo traz um slide de abertura com a palavra "Título" esperando alguém.
 * Ele saía assim no documento entregue — e um deck que abre escrito "Título" é
 * o tipo de coisa que ninguém vê no ensaio e todo mundo vê na reunião.
 *
 * A quebra é de verdade, com <a:br/>: ver `corpoComQuebras` em `pptx.js`.
 */
export const TITULO_CAPA = 'S&OP\nCAPACITY TOOL';
...
// Com o sinal explícito: "+3.150" e "−1.234" se leem sozinhos como sobra e
// falta, e o "+" é o que impede o positivo de parecer um número qualquer.
// Zero fica "0". O menos é o tipográfico (U+2212): o hífen se perde numa
// grade de números arredondados.
export const fmtSinal = (n) => {
...
/**
 * "Ocupação (ideal)" — o nome da faixa em que o consolidado do ano caiu.
 *
 * Sem o nome, o slide depende da COR, e a legenda das cores não está nele: quem
 * assiste teria que lembrar que verde é bom e amarelo é apertado. Escrito, o
 * rótulo responde sozinho.
 *
 * Volta "Ocupação" seco quando não há faixa cadastrada que cubra o valor, ou
 * quando não há ocupação nenhuma — parêntese vazio é pior que parêntese nenhum.
 */
export const rotuloOcupacao = (faixas, pct) => {
  const nome = faixaDe(faixas, pct)?.rotulo;
  return nome ? `Ocupação (${String(nome).trim()})` : 'Ocupação';
};
 ...
 *
 * `ABERTO` devolve a APRESENTAÇÃO INTEIRA em ordem de leitura: a planta, a
 * área, e então, para cada centro de custo, o consolidado dele seguido dos CTs
 * que o compõem. Cada grupo carrega a própria `granularidade`, e é isso que faz
 * o resto funcionar sem saber de nada: quem monta o slide pergunta ao grupo em
 * que nível ele está, em vez de assumir que todos estão no mesmo.
 *
 * A ordem importa e é o pedido: consolidado ANTES da abertura. Quem assiste vê
 * o número do CC e só então de onde ele veio — o contrário obriga a somar de
 * cabeça enquanto os slides passam.
 */
export function agrupa(linhas, granularidade = 'RESUMO') {
  const lista = linhas ?? [];
  if (!lista.length) return [];

  if (granularidade === 'ABERTO') {
    const saida = NIVEIS_ABERTO.flatMap((n) => agrupa(lista, n));
    for (const cc of agrupa(lista, 'CC')) {
      saida.push(cc);
      // Só as linhas DESTE centro de custo abrem em CT. Reabrir a lista inteira
      // a cada CC repetiria todos os CTs debaixo de todos os CCs.
      saida.push(...agrupa(
        lista.filter((l) => chaveDoGrupo(l, 'CC') === cc.chave), 'CT'));
    }
    return saida;
  }
  ...
  return ordem.map((k) => {
    ...
    return {
      ...
      // O CC só existe onde ele identifica o grupo. Numa planta ou numa área
      // ele seria o primeiro de quarenta, e um subtítulo dizendo "CC 163" num
      // slide que soma a fábrica inteira é uma legenda mentindo.
      cc: granularidade === 'CT' || granularidade === 'CC' ? p.cc : null,
      planta: plantas.join(' · '),
      area: areas.join(' · '),
      ccs: ccs.length,
...
export const chaveDoGrupo = (l, granularidade) =>
  (granularidade === 'CT' ? l.ct
    ...
    : granularidade === 'AREA' ? `${l.planta}|${l.area}`
    : granularidade === 'PLANTA' ? String(l.planta)
    : '*');
...
export function visualDoGrupo({
  ...
  const linhasDeTurno = listaTurnos.map((t) => ({
    ...
    valores: meses.map((m) => {
      ...
      // VAZIO, e não "N/A". Saiu como N/A de 03/09 a 15/09/2026, com o
      // argumento de que célula em branco é ambígua entre "não tem" e "não
      // consegui contar". Na prática, com sete turnos cadastrados e dois em
      // uso, o slide virava uma grade de N/A e a linha que importava sumia no
      // meio. A linha do turno continua lá — é ela que diz "o 3º não roda
      // aqui" —, e a célula em branco é o que se lê como "não roda".
      return q > 0 ? fmt(q) : '';
    }),
  }));
  ...
  return {
    ...
    linhas: [
      ...
      ...(cenario ? [
        ...
        { grupo: 'cálculo',
          // A FAIXA DO ANO NO RÓTULO: "Ocupação (ideal)", "Ocupação
          // (estourado)". A cor já diz isso mês a mês, mas cor sozinha exige
          // que alguém lembre a legenda — e a legenda não está no slide. O
          // nome escrito responde a pergunta que a apresentação inteira existe
          // para responder, e responde pelo ANO, que é a conclusão.
          //
          // Do TOTAL e não do mês: doze faixas no rótulo não caberiam, e o mês
          // continua colorido para quem quiser o detalhe.
          rotulo: rotuloOcupacao(faixas, total.ocupacao),
        ...
        // SOBRA / FALTA: a mesma distância que o painel da ocupação escreve
        // embaixo do indicador ("sobra 3.150 min"). A porcentagem diz se cabe;
        // este número diz QUANTO falta contratar ou quanto sobra para vender,
        // e é ele que vai para a conversa com a fábrica. Positivo é sobra,
        // negativo é falta — o sinal é a leitura, e o rótulo diz a ordem.
        { grupo: 'cálculo',
          rotulo: `Sobra / falta (${rotuloUnidade(un)})`,
          valores: pontos.map((p) => fmtSinal(p.capacidade - p.demanda)),
          total: fmtSinal(total.capacidade - total.demanda) },
      ] : []),
 ...
 */
export const tituloDoSlide = (g) => (g.granularidade === 'PLANTA'
  // Na planta o título é só ela: juntar as áreas ali daria "Matriz - Tecelagem ·
  // Confecção · Beneficiamento", que é a lista do que está somado e não o nome
  // de quem soma. A lista vai para o subtítulo, que é onde ela informa.
  ? String(g.planta)
  : `${g.planta} - ${g.area}`);
...
export function subtituloDoSlide(g) {
  ...
  if (g.granularidade === 'PLANTA') {
    return `${g.area} · ${fmt(g.ccs)} CCs - ${fmt(g.cts)} CTs`;
  }
...
export function tituloDoGrupo(g) {
  ...
  if (g.granularidade === 'AREA') return `${g.area} · ${g.planta}`;
  if (g.granularidade === 'PLANTA') return String(g.planta);
