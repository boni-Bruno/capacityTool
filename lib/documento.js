...
export function visualDoGrupo({
  ...
  const pontos = meses.map((m) => {
    ...
    return {
      ...
      capacidade: soma(linhas, medida),
      demanda: cenario ? soma(linhas, 'demanda') : 0,
  ...
  // Um turno que só existe em parte do ano aparece com a coluna vazia nos meses
  // em que não valia — some da linha, não do slide.
  const nomes = [...new Set((turnos ?? [])
    .filter((l) => chaveDoGrupo(l, granularidade) === chave)
    .map((l) => l.turno))].sort((a, b) => String(a).localeCompare(String(b), 'pt-BR'));

  const linhasDeTurno = nomes.map((nome) => ({
    rotulo: nome,
    valores: meses.map((m) => {
      const q = (dosTurnos.get(m) ?? [])
        .filter((l) => l.turno === nome)
  ...
  return {
    ...
    linhas: [
      { rotulo: '', valores: pontos.map((p) => p.rotulo), cabecalho: true },
      { rotulo: 'OEE', valores: pontos.map((p) => fmtPct(p.oee)) },
      ...linhasDeTurno,
    ],
    rotuloCapacidade: rotuloMedida(medida),
    rotuloDemanda: cenario ? `Demanda ${cenario}` : null,
  };
}
