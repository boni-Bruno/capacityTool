...
export function visualDoGrupo({
  ...
  return {
    ...
    linhas: [
      ...
      // Ao lado do OEE porque as duas explicam a mesma coisa: por que a
      // capacidade daquele mês é a que é. O OEE tira um percentual do que
      // sobrou; a parada tira minutos antes disso.
      { rotulo: 'Paradas', valores: pontos.map((p) => fmt(p.parada)),
        total: fmt(total.parada) },
