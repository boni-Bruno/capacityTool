...
import {
  ...
  demandaPorAtributo, fatiasDoRotulo, indiceDoMixManual, mixDaBase,
  podeSerCondicao, previa, primeiraQueCasa, regraCasa, rotulosDe,
} from './regras.js';
...
// --- a demanda por rótulo --------------------------------------------------
//
// Sem rateio nenhum: a demanda É da linha, e a linha É classificada. Somar por
// rótulo é ler o que a base já diz.

test('a demanda vai inteira para o rótulo da linha', () => {
  const d = demandaPorAtributo(CAMA_E_DECORACAO, ATTR, R2, 'linha');
  assert.equal(d.find((x) => x.rotulo === 'Cama').meses.get(MES), 1000);
  assert.equal(d.find((x) => x.rotulo === 'Decoração').meses.get(MES), 1000);
});

test('o que nenhuma regra classifica vira o rótulo nulo, e não some', () => {
  const outro = [{ ...LINHA, ct: '515-004', mes: MES, familia_tecelagem: '999',
                   minutos: 300, metros: 0, qtd: 0 }];
  const d = demandaPorAtributo([...CAMA_E_DECORACAO, ...outro], ATTR, R2, 'linha');
  assert.equal(d.find((x) => x.rotulo === null).meses.get(MES), 300);
});

test('a lista de CTs limita a soma à seleção da tela', () => {
  const fora = [{ ...LINHA, ct: '401-003', mes: MES, familia_tecelagem: '225',
                  minutos: 500, metros: 0, qtd: 0 }];
  const d = demandaPorAtributo([...CAMA_E_DECORACAO, ...fora], ATTR, R2, 'linha',
    ['515-004']);
  assert.equal(d.find((x) => x.rotulo === 'Cama').meses.get(MES), 1000);
});
