...
//                             paradas e sábado curto — SEM o OEE.
...
// muda-se 18 para 22, ou 65% para 70%, e tudo à direita responde. Como dividir
// as 22 entre os turnos é decisão de quem cadastra, depois — abrir a planilha
// por turno só espalharia a mesma pergunta em mais linhas.
//
 ...
 *
 * Demanda, dias úteis, minutos planejados por unidade, OEE e unidades saem
 * como VALOR — os dois últimos em célula de entrada. Minutos disponíveis por
 * unidade, disponível e ocupação são FÓRMULA, para a planilha responder quando
 * alguém mexe nas entradas.
 */
export function montarSimulador(linhas) {
  ...
  for (const ct of cts) {
    ...
    ct.meses.forEach((m, i) => {
      ...
      linhasCt.push([
        ...
        { f: `J${r}*K${r}`,                      estilo: ESTILO.DECIMAL },
        { v: f.unidades,         estilo: ESTILO.ENTRADA },
        { f: `M${r}*L${r}*I${r}`,                estilo: ESTILO.INTEIRO },
        { f: `IF(N${r}=0,"",H${r}/N${r})`,       estilo: ESTILO.PERCENTUAL },
      ]);
      r += 1;
    });
    ...
    // ano é disponível ÷ planejada, e a planejada é M×J×I somado.
    const unidDias = `SUMPRODUCT(M${ini}:M${fim},I${ini}:I${fim})`;
    const planejada = `SUMPRODUCT(J${ini}:J${fim},M${ini}:M${fim},I${ini}:I${fim})`;
    linhasCt.push([
      ...
      { f: `IF(${planejada}=0,"",N${r}/${planejada})`,      estilo: ESTILO.PERCENTUAL },
      { f: `IF(${unidDias}=0,"",N${r}/${unidDias})`,        estilo: ESTILO.DECIMAL },
  ...
  for (const cc of ccs.values()) {
    const linhaCc = (mes, rotulo) => [
      ...
      { f: somaDe('M', r2, mes), estilo: ESTILO.DECIMAL },
      { f: somaDe('N', r2, mes), estilo: ESTILO.INTEIRO },
...
export function resumoDoSimulador(linhas) {
  return porCt(linhas).map((ct) => {
    ...
    return {
      ...
      minDisponiveisPorUnidadeDia: unidadeDias > 0 ? disponivel / unidadeDias : null,
