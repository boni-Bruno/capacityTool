...
// =============================================================================
// O TÍTULO E O SUBTÍTULO DO PRÓPRIO MODELO
//
// O modelo já tem onde escrever o título — é a caixa que aparece escrito
// "Título" quando está vazia. Escrever o título dentro da caixa do conteúdo,
// como se ela não existisse, gasta a altura de que o desenho precisa e ignora a
// posição, a fonte e o tamanho que o modelo já decidiu para ele.
//
// A BUSCA É POR TIPO DE ESPAÇO RESERVADO, e não por posição: `type="title"` é o
// que o PowerPoint marca na caixa do título, seja qual for o leiaute. Como
// alternativa, vale a caixa comum em que alguém escreveu literalmente "Título"
// — que é como um modelo montado à mão costuma ficar.
// =============================================================================

// Comparação que ignora acento e caixa, sem passar por classe de marca
// combinante: escrever esses caracteres no arquivo é o tipo de coisa que não
// sobrevive a uma troca de codificação, e o que sobra é um regex que não casa
// com nada — em silêncio. Aqui quem sabe o que é acento é o próprio idioma.
const mesmoRotulo = (a, b) => String(a ?? '').trim()
  .localeCompare(String(b), 'pt-BR', { sensitivity: 'base' }) === 0;

const TIPOS = {
  titulo: { tipos: ['ctrTitle', 'title'], rotulos: ['titulo'] },
  subtitulo: { tipos: ['subTitle'], rotulos: ['subtitulo', 'sub-titulo'] },
};

/** O texto visível de uma forma, já costurado. */
const textoDaForma = (sp) =>
  [...costura(sp).matchAll(/<a:t>([^<]*)<\/a:t>/g)].map((m) => m[1]).join('');

/**
 * Escreve num campo do modelo — o título ou o subtítulo.
 *
 * Caixa VAZIA não tem `<a:t>` nenhum para clonar: o "Título" que aparece na
 * tela é sugestão do leiaute, não conteúdo do arquivo. Nesse caso entra um
 * parágrafo mínimo, sem fonte nem tamanho — e é justamente aí que o espaço
 * reservado brilha: ele herda tudo do leiaute, então o resultado sai igual ao
 * que sairia se alguém tivesse digitado ali.
 */
/**
 * QUEBRA DE LINHA no DrawingML e um ELEMENTO, e nao um caractere.
 *
 * O <a:t> guarda texto corrido: uma nova linha ali vira espaco na hora de
 * desenhar, e o titulo sairia numa linha so — sem erro nenhum para denunciar. A
 * quebra de verdade e o <a:br/> entre dois runs, e os dois runs precisam
 * repetir as MESMAS propriedades de fonte: senao a segunda linha herda o padrao
 * do leiaute e o titulo sai meio negrito, meio nao.
 *
 * O rPr chega de fora porque quem o encontrou foi quem achou o paragrafo.
 *
 * O corte aceita o par do Windows tambem: texto colado de outro lugar traz CR
 * antes do LF, e cortar so no LF deixaria um CR solto dentro do XML.
 */
const corpoComQuebras = (conteudo, rPr = '') => String(conteudo ?? '')
  .split(/\r?\n/)
  .map(escapa)
  .join(`</a:t></a:r><a:br/><a:r>${rPr}<a:t>`);

export function preencheCampo(xml, qual, conteudo) {
  const alvo = TIPOS[qual];
  if (!alvo) return { xml: decodifica(xml), trocou: false };

  const t = costura(decodifica(xml));
  for (const m of t.matchAll(/<p:sp>[\s\S]*?<\/p:sp>/g)) {
    const sp = m[0];
    const ph = sp.match(/<p:ph[^>]*type="([^"]+)"/)?.[1];
    const combina = (ph && alvo.tipos.includes(ph))
      || alvo.rotulos.some((r) => mesmoRotulo(textoDaForma(sp), r));
    if (!combina) continue;

    const onde = sp.indexOf('<a:t>');
    let novo;
    if (onde >= 0) {
      const lim = limitesDoParagrafo(sp, onde);
      if (!lim) continue;
      const paragrafoVelho = sp.slice(lim.inicio, lim.fim);
      // As propriedades do run que já existe, para as linhas novas saírem com a
      // mesma fonte. Sem isto a segunda linha herda o padrão do leiaute.
      const rPr = paragrafoVelho.match(
        /<a:rPr[^>]*\/>|<a:rPr[\s\S]*?<\/a:rPr>/)?.[0] ?? '';
      novo = sp.slice(0, lim.inicio)
        + paragrafoVelho.replace(/(<a:t>)[\s\S]*?(<\/a:t>)/,
                                 `$1${corpoComQuebras(conteudo, rPr)}$2`)
        + sp.slice(lim.fim);
    } else {
      const rPr = '<a:rPr lang="pt-BR" dirty="0"/>';
      const paragrafo = `<a:p><a:r>${rPr}`
        + `<a:t>${corpoComQuebras(conteudo, rPr)}</a:t></a:r></a:p>`;
      novo = /<a:p>[\s\S]*?<\/a:p>/.test(sp)
        ? sp.replace(/<a:p>[\s\S]*?<\/a:p>/, paragrafo)
        : sp.replace('</p:txBody>', `${paragrafo}</p:txBody>`);
    }

    return { xml: t.slice(0, m.index) + novo + t.slice(m.index + sp.length),
             trocou: true };
  }

  return { xml: t, trocou: false };
}
...
/**
 * O RETÂNGULO DE UM CAMPO DO MODELO — hoje o subtítulo, para a pílula do
 * período subir e ficar na linha dele.
 *
 * PROCURA NO SLIDE E DEPOIS NO LEIAUTE, e a segunda busca é a que costuma
 * achar: espaço reservado quase nunca declara a própria posição no slide — ele
 * a HERDA do leiaute, e é por isso que arrastar o título num slide não move o
 * dos outros. Parar na primeira busca devolveria nulo quase sempre, e a pílula
 * nunca sairia do lugar.
 *
 * Nulo quando nem o leiaute declara. Quem chama decide; aqui não dá para
 * adivinhar, e chutar uma altura pintaria por cima do texto do modelo.
 */
export function retanguloDoCampo(arquivos, nomeDoSlide, qual) {
  const alvo = TIPOS[qual];
  if (!alvo) return null;

  // O MESMO CASAMENTO DE `preencheCampo`, e isto não é detalhe: por tipo E por
  // rótulo. Um modelo montado à mão não tem espaço reservado nenhum — tem uma
  // caixa de texto onde alguém escreveu "Subtítulo" —, e casar só por tipo
  // devolvia nulo justamente no modelo que está em uso. O sintoma foi mudo: a
  // pílula não subia, o desenho não crescia, e nada acusava o porquê.
  const naParte = (xml) => {
    for (const m of costura(decodifica(xml)).matchAll(/<p:sp>[\s\S]*?<\/p:sp>/g)) {
      const ph = m[0].match(/<p:ph[^>]*type="([^"]+)"/)?.[1];
      const combina = (ph && alvo.tipos.includes(ph))
        || alvo.rotulos.some((r) => mesmoRotulo(textoDaForma(m[0]), r));
      if (!combina) continue;
      const off = m[0].match(/<a:off x="(-?\d+)" y="(-?\d+)"\s*\/>/);
      const ext = m[0].match(/<a:ext cx="(\d+)" cy="(\d+)"\s*\/>/);
      if (!off || !ext) return null;
  ...
  const doSlide = naParte(arquivos.get(nomeDoSlide) ?? '');
  if (doSlide) return doSlide;

  // O leiaute sai do .rels do slide, e não de um nome montado por convenção:
  // slide3.xml não usa slideLayout3.xml, usa aquele para o qual ele aponta.
  const rels = decodifica(
    arquivos.get(nomeDoSlide.replace(/^ppt\/slides\//, 'ppt/slides/_rels/') + '.rels')
    ?? '');
  const alvoRel = rels.match(/Target="([^"]*slideLayout\d+\.xml)"/)?.[1];
  if (!alvoRel) return null;

  const nomeLeiaute = 'ppt/slideLayouts/' + alvoRel.replace(/^.*\//, '');
  return naParte(arquivos.get(nomeLeiaute) ?? '');
}
 ...
 * porque quem monta o parágrafo é quem sabe como o modelo o formata.
 */
