...
// Turnos que o recurso faz em `data`, com todos os turnos da planta ao lado
// para a tela poder ligar/desligar cada um.
export async function turnosDoRecurso(recursoId, data) {
  return sql`
    select t.id                    as turno_id,
           t.codigo,
           t.nome,
           rt.id                   as vinculo_id,
           lower(rt.vigencia)::text as vigente_desde
      from turno t
      left join recurso_turno rt
             on rt.turno_id   = t.id
            and rt.recurso_id = ${recursoId}
            and rt.vigencia  @> ${data}::date
     where t.ativo
     order by t.codigo`;
}

// Grade turno x mês do ano inteiro, para o recurso escolhido.
//
// Devolve quantos dias de cada mês estão cobertos por alguma vigência, não só
// sim/não: assim um turno que liga no dia 15 aparece como mês parcial em vez
// de mentir "ligado o mês todo".
//
...
export async function gradeAnualTurnos(recursoId, ano) {
  return sql`
    with m as (
...
// As faixas cruas, para conferir o histórico e poder apagar cadastro errado.
export async function vigenciasDoRecurso(recursoId) {
  return sql`
    select rt.id,
           t.codigo,
           t.nome,
           lower(rt.vigencia)::text as inicio,
           upper(rt.vigencia)::text as fim
      from recurso_turno rt
      join turno t on t.id = rt.turno_id
     where rt.recurso_id = ${recursoId}
     order by t.codigo, lower(rt.vigencia)`;
}
