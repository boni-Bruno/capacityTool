...
/**
 * QUAIS TURNOS PODEM SER OFERECIDOS A ESTES RECURSOS.
 *
 * Os ativos DA PLANTA deles, mais qualquer um que eles já usem. As duas metades
 * existem por motivos diferentes:
 *
 * A planta, porque a tela oferecia turno de outra: a Matriz mostrava o "2º
 * Turno" de Ibirama, e sessenta e cinco recursos da Matriz acabaram cadastrados
 * nele. Nenhum número saiu errado — os horários dos dois eram idênticos —, mas
 * era coincidência, e coincidência não é cadastro.
 *
 * E o "já usa", porque salvar REESCREVE o ano: turno que some da tela sai do
 * `marcados`, e o save o apagaria por omissão. Uma correção de coluna não pode
 * apagar o cadastro de sessenta e cinco recursos de lambuja.
 */
const TURNOS_OFERECIDOS = (recursos) => sql`
  select t.id, t.codigo, t.nome
    from turno t
   where (t.ativo and t.planta_id in (
            select a.planta_id from recurso r
              join area a on a.id = r.area_id
             where r.id = any(${recursos}::int[])))
      or exists (select 1 from recurso_turno rt
                  where rt.turno_id = t.id
                    and rt.recurso_id = any(${recursos}::int[]))
   order by t.codigo`;
...
export async function matrizTurnosDoAno(recursoId, ano) {
  return sql`
    with turnos_do_recurso as (
      select t.id
        from turno t
       where (t.ativo and t.planta_id = (
                select a.planta_id from recurso r
                  join area a on a.id = r.area_id
                 where r.id = ${recursoId}))
          or exists (select 1 from recurso_turno rt
                      where rt.turno_id = t.id
                        and rt.recurso_id = ${recursoId})
    ),
    m as (
