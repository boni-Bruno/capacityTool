...
/**
 * Em quais calendários o turno roda, e em que dias.
 *
 * calendario_regra diz "neste calendário, neste dia da semana, roda este
 * turno". O motor exige a linha: sem ela, calendario_ok é falso e a capacidade
 * sai zero mesmo com o turno marcado no recurso.
 *
 * O seed criou regra só para os turnos que existiam naquele momento — turno
 * criado depois nasce fora de todos os calendários, o que é uma armadilha
 * silenciosa.
 */
export async function calendariosDoTurno(turnoId) {
  return sql`
    select c.id,
           c.nome,
           (select string_agg(distinct cr.dia_semana::text, ','
             ...
             where cr.calendario_id = c.id and cr.turno_id = ${Number(turnoId)}
           ) as dias_do_turno,
           (select string_agg(distinct cr.dia_semana::text, ','
                              order by cr.dia_semana::text)
              from calendario_regra cr
             where cr.calendario_id = c.id
           ) as dias_do_calendario
      from turno t
      join calendario c on c.planta_id = t.planta_id
     where t.id = ${Number(turnoId)}
     order by c.codigo`;
}

/**
 * Coloca o turno em todos os calendários da planta, nos dias que cada
 * calendário já opera.
 *
 * Os dias vêm das regras que o calendário já tem — é a única definição de
 * "dias do calendário" que existe no modelo. Calendário ainda sem nenhuma
 * regra recebe segunda a sábado, que é o padrão da casa; domingo só entra em
 * calendário que já trabalha domingo, para não transformar PADRAO em RODIZIO
 * por acidente.
 */
export async function incluirTurnoNosCalendarios(turnoId) {
  const t = Number(turnoId);
  const cals = await calendariosDoTurno(t);
  const passos = [];

  for (const c of cals) {
    const dias = c.dias_do_calendario
      ? c.dias_do_calendario.split(',').map(Number)
      : [1, 2, 3, 4, 5, 6];
    const jaTem = new Set(
      (c.dias_do_turno ?? '').split(',').filter(Boolean).map(Number));
...
export async function criarTurno({ planta_id, codigo, nome }) {
  ...
  // Turno que não está em calendário nenhum produz zero, mesmo marcado no
  // recurso. Já nasce nos calendários da planta para não virar armadilha.
  await incluirTurnoNosCalendarios(r[0].id);
  return r[0].id;
}
 ...
 *
 * Se o turno já foi usado em qualquer outro lugar — calendário, recurso,
 * parada, exceção, OEE ou numa rodada de cálculo — apagar arrancaria a
...
export async function excluirTurno(id) {
  ...
  const r = await sql`
    select
      (select count(*) from calendario_regra where turno_id = ${turnoId}) as calendarios,
  ...
  const usos = r[0];
  const onde = [];
  if (Number(usos.calendarios)) onde.push('calendário');
  ...
  return sql`
    select c.id,
           ...
           (select string_agg(distinct cr.dia_semana::text, ','
                              order by cr.dia_semana::text)
              from calendario_regra cr
             where cr.calendario_id = c.id) as dias
      from recurso r
