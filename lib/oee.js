import { sql } from './db';
import { mesesDoAno, recomporFaixasComValor } from './faixas';
...
export async function aplicarOeeEmLote(recursoIds, ano, origem, porMes) {
  ...
  // Só o que foi digitado. Validar ANTES de tocar em qualquer recurso: um OEE
  // inválido no meio do lote deixaria metade aplicada e metade não.
  const aplicar = {};
  ...
  let faixas = 0;
  for (const id of ids) {
    const atuais = mesesDoAno(await faixasOee(id, origem), ano);
    const r = await definirOeeDoAno(id, ano, origem, { ...atuais, ...aplicar });
    faixas += r.faixas;
  }
