...
// -----------------------------------------------------------------------------
// ESCOPO DAS ROTAS → o que no corpo diz ONDE a escrita acontece.
//
// Cada entrada é { tipo, campo }: `campo` é a chave do corpo JSON, `tipo` diz
// como resolvê-la para uma área ou planta (lib/sessao.js faz a busca):
//   area          o valor é o id da área
//   areaDoRecurso o valor é o id de um recurso → a área dele
//   areasDosRecursos  lista de ids de recurso (lote)
//   areaDaParada  id de parada → recurso → área
//   planta        o valor é o id da planta (o que é da planta pede a planta inteira)
//   plantaDaArea, plantaDoTurno, plantaDoCalendario, plantaDaExcecao  id → planta
//   empresa       só quem tem a empresa inteira (criar planta)
// Rota sem entrada aqui não tem escopo — é da empresa (demanda, DE/PARA, mix,
// cargos, usuários, extrações), e só a permissão decide.
// -----------------------------------------------------------------------------
export const ESCOPO_ROTAS = {
  'cadastro/planta':            { POST: { tipo: 'empresa' }, '*': { tipo: 'planta', campo: 'id' } },
  'cadastro/area':              { POST: { tipo: 'planta', campo: 'planta_id' }, '*': { tipo: 'plantaDaArea', campo: 'id' } },
  'cadastro/recurso':           { POST: { tipo: 'area', campo: 'area_id' }, '*': { tipo: 'areaDoRecurso', campo: 'id' } },
  'cadastro/recurso-definitivo': { '*': { tipo: 'areaDoRecurso', campo: 'id' } },
  'cadastro/recurso-turno':     { '*': { tipo: 'areaDoRecurso', campo: 'recurso_id' } },
  'cadastro/recurso-calendario': { '*': { tipo: 'areaDoRecurso', campo: 'recurso_id' } },
  'cadastro/oee':               { '*': [{ tipo: 'areaDoRecurso', campo: 'recurso_id' }, { tipo: 'areasDosRecursos', campo: 'recursos' }] },
  'cadastro/parada':            { POST: { tipo: 'areaDoRecurso', campo: 'recurso_id' }, '*': { tipo: 'areaDaParada', campo: 'id' } },
  'cadastro/turno':             { POST: { tipo: 'planta', campo: 'planta_id' }, '*': { tipo: 'plantaDoTurno', campo: 'id' } },
  'cadastro/turno-horario':     { '*': { tipo: 'plantaDoTurno', campo: 'turno_id' } },
  'cadastro/calendario':        { POST: { tipo: 'planta', campo: 'planta_id' }, '*': { tipo: 'plantaDoCalendario', campo: 'id' } },
  'cadastro/calendario-copia':  { '*': { tipo: 'planta', campo: 'planta_destino_id' } },
  'cadastro/calendario-peso':   { '*': { tipo: 'plantaDoCalendario', campo: 'calendario_id' } },
  'cadastro/calendario-regra':  { '*': { tipo: 'plantaDoCalendario', campo: 'calendario_id' } },
  'cadastro/excecao':           { POST: { tipo: 'planta', campo: 'planta_id' }, '*': { tipo: 'plantaDaExcecao', campo: 'id' } },
  'recalcular':                 { POST: { tipo: 'area', campo: 'areaId' } },
};

/** As exigências de escopo de uma rota e método, sempre como lista. */
export function escopoDaRota(pasta, metodo = 'POST') {
  const v = ESCOPO_ROTAS[pasta];
  if (!v) return [];
  const e = v[metodo] ?? v['*'];
  if (!e) return [];
