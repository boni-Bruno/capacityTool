...
test('célula vazia em tipo, qtd, equivalência e ativo mantém o que o recurso já tem', () => {
  ...
  ], { areas: AREAS, existentes: [{ ...EXISTENTE, tipo_recurso: 'PESSOA', qt_recursos: 1, ativo: false }] });
  assert.equal(r.iguais, 1);
  assert.equal(r.alterar.length, 0);
});
...
test('pessoa não tem Qtd: sai vazia e, na volta, qualquer coisa na célula vira 1', () => {
  const [l] = linhasParaExportar([{ ...EXISTENTE, tipo_recurso: 'PESSOA', qt_recursos: 1 }]);
  assert.equal(l[COLUNAS.findIndex((c) => c.chave === 'qt_recursos')], '');

  const r = montarImportacao([
    CAB,
    linha({ ...base, patrimonio: '9', tipo: 'pessoa', qt_recursos: 22 }),
    linha({ ...base, patrimonio: '10', tipo: 'pessoa', qt_recursos: 'vinte' }),
  ], { areas: AREAS, existentes: [] });
  assert.deepEqual(r.erros, []);
  assert.equal(r.criar[0].qt_recursos, 1);
  assert.equal(r.criar[1].qt_recursos, 1);

  // Pessoa existente com Qtd 1 e planilha com 22: igual, nada a alterar.
  const s = montarImportacao([
    CAB,
    linha({ ...base, tipo: 'pessoa', qt_recursos: 22 }),
  ], { areas: AREAS, existentes: [{ ...EXISTENTE, tipo_recurso: 'PESSOA', qt_recursos: 1 }] });
  assert.equal(s.iguais, 1);
});
