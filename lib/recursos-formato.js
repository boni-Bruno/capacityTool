...
export function linhasParaExportar(itens) {
  return (itens ?? []).map((r) => [
    ...
    r.ct ?? '',
    r.patrimonio ?? '',
    // Pessoa não tem Qtd (migração 36): a célula sai vazia, como na tela, e
    // na volta o que estiver ali é ignorado.
    r.tipo_recurso === 'PESSOA' ? '' : Number(r.qt_recursos ?? 1),
...
export function montarImportacao(linhas, { areas, existentes }) {
  ...
  for (let i = 1; i < linhas.length; i += 1) {
    ...
    const tipoFinal = tipo ?? existente?.tipo_recurso ?? 'MAQUINA';

    // Pessoa não tem Qtd: o que vier na célula é ignorado e grava 1, como o
    // servidor faria de qualquer jeito. Validar um número que não vai ser
    // usado só devolveria erro sobre uma coluna que não se aplica.
    const qt = tipoFinal === 'PESSOA' ? 1 : leNumero(col('qt_recursos'));
