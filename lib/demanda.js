...
// Regra cadastrada que virou obsoleta: o CT ganhou demanda própria mas continua
// herdando, porque a regra ganha por decisão. Não é erro — é o caso que a
// premissa cria, e ele precisa aparecer para poder ser revisto.
export async function heranciasComDemandaPropria(cargaId) {
  return sql`
    select o.ct, o.tipo, o.valor,
      ...
      join vw_demanda_indice i on i.ct = o.ct and i.carga_id = ${Number(cargaId)}
     where o.tipo in ('CT', 'CC')
     group by o.ct, o.tipo, o.valor
     order by 5 desc`;
}
