...
/**
 * A anotação do cenário — ver 30_carga_observacao.sql.
 *
 * Vazio APAGA a anotação em vez de gravar string vazia: "nulo" e "duas aspas"
 * seriam dois jeitos de dizer a mesma coisa, e a tela teria que tratar os dois.
 *
 * Não passa por revalidação de cálculo nenhum: é texto ao lado do número, e não
 * entra em conta alguma.
 */
export async function anotarCarga(cargaId, observacao) {
  const texto = String(observacao ?? '').trim();
  const r = await sql`
    update demanda_carga
       set observacao = ${texto || null}
     where id = ${Number(cargaId)}
...
export async function cargas() {
  return sql`
    select c.id, c.arquivo, c.cenario, c.corrente, c.linhas, c.criado_por,
           c.extraido_em, c.criado_em, c.observacao,
 ...
 */
export async function combinacoesPorMes(cargaId, campos = [], cts = null) {
  const usa = (c) => campos.includes(c);
  // `cts` estreita a leitura aos centros que interessam. O índice do mix manual
  // só vale para quem tem mix cadastrado — geralmente um punhado —, e varrer a
  // base inteira por causa deles seria pagar caro por nada.
  const lista = cts?.length ? cts.join(',') : null;
  return sql`
    select case when ${usa('grupo_estoque')} then l.grupo_estoque end
     ...
     where l.carga_id = ${Number(cargaId)}
       ...
       and (${lista}::text is null
            or l.ct = any(string_to_array(${lista}, ',')))
...
/**
 * Todo o mix ajustado de um ano, de todos os atributos.
 *
 * O painel precisa saber se existe QUALQUER ajuste antes de decidir se vale a
 * pena ler as combinações — a consulta cara. Esta é barata: a tabela tem uma
 * linha por CT, mês e rótulo ajustado, e quem nunca ajustou nada não paga nada.
 */
export async function mixAjustesDoAno(ano) {
  return sql`
    select atributo, ct, mes, rotulo, pct::float8 as pct
      from mix_ajuste
     where ano = ${Number(ano)}
     order by atributo, ct, mes`;
}

/** Os apontamentos de taxa de todos os atributos. */
export async function todasAsTaxasDeMix() {
  return sql`select ct, atributo, tipo, valor from mix_taxa`;
}
