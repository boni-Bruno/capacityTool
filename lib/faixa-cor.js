...
/**
 * Preto ou branco por cima da cor, pelo que se lê melhor.
 *
 * Texto escuro sobre vermelho escuro é uma célula ilegível, e ilegível num
 * slide projetado é pior que sem cor nenhuma.
 *
 * SEM LIMIAR CHUTADO: o contraste dos dois candidatos é calculado e ganha o
 * maior. Um corte fixo na luminância erra justamente no meio da escala — âmbar
 * fica com luz 0,48, que "parece escuro" perto de 1, mas contrasta seis vezes
 * melhor com preto do que com branco. A conta é a do sRGB, a mesma das regras
 * de acessibilidade.
 */
export function corDoTexto(fundo) {
  const c = normalizaCor(fundo);
  if (!c) return null;
  const canal = (i) => {
  ...
  const contraPreto = (luz + 0.05) / 0.05;
