...
test('todos os turnos cadastrados aparecem, e o que não roda diz N/A', () => {
  ...
  assert.deepEqual(v.linhas.slice(-3).map((l) => l.rotulo),
...
test('sem a lista de turnos, cai no que o recorte tem', () => {
  ...
  const v = visual();
  assert.deepEqual(v.linhas.slice(-1).map((l) => l.rotulo), ['1º turno']);
});
