...
// `permissoesDoCargo` morava aqui e era uma segunda consulta na montagem da
// sessão. Saiu quando `sessaoDoUsuario` passou a trazer cargo, permissões e
// escopo junto com o usuário — o '*' do cargo protegido é resolvido lá.
...
/**
 * TUDO O QUE A SESSÃO PRECISA, NUMA CONSULTA SÓ.
 *
 * Eram quatro — usuário, cargo, permissões, escopo — em duas idas ao banco, e
 * cada ida é um round trip inteiro do driver HTTP do Neon. Numa tela que faz
 * dez consultas em série, três delas serem de sessão é o que se paga antes de
 * a página começar.
 *
 * Não traz o hash da senha: a sessão não confere senha, e carregar hash em
 * toda requisição é carga que não serve a ninguém.
 *
 * `permissoes` vem nulo para o cargo protegido — quem lê resolve para '*',
 * porque o protegido tem também as telas que ainda não existem.
 */
export async function sessaoDoUsuario(id) {
  const r = await sql`
    select u.id, u.login, u.nome, u.email, u.ativo, u.trocar_senha,
           u.cargo_id, c.nome as cargo, c.protegido,
           case when c.protegido then null else coalesce(
             (select array_agg(cp.codigo order by cp.codigo)
                from cargo_permissao cp where cp.cargo_id = c.id), '{}') end
             as permissoes,
           coalesce((select json_agg(json_build_object(
                       'nivel', e.nivel, 'referencia_id', e.referencia_id))
                       from usuario_escopo e where e.usuario_id = u.id), '[]')
             as escopo
      from usuario u
      join cargo c on c.id = u.cargo_id
     where u.id = ${Number(id)}`;
...
// `escopoDe` saiu pelo mesmo motivo que `permissoesDoCargo`: era a terceira
// consulta da sessão, e agora vem junto em `sessaoDoUsuario`. A tela de
// usuários já lia o escopo de todos numa consulta só.
