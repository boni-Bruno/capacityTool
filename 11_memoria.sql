...
begin
    ...
    select c.recurso_id, c.planta_id, c.area_id, c.data, c.turno_id,
           ...
           vtm.duracao_turno::bigint                  as v_bruto,
           vtm.minutos::bigint                        as v_liquido,
           (vtm.minutos * c.qt_recursos * c.equivalencia)::bigint as v_quantidade,
           case when not (c.calendario_ok and c.escala_ok) then 0::bigint
                else (vtm.minutos * c.qt_recursos * c.equivalencia)::bigint
           end                                        as v_calendario,
           case when not (c.calendario_ok and c.escala_ok)
                  or coalesce(pa.tem_dia_inteiro, false) then 0::bigint
                else (vtm.minutos * c.qt_recursos * c.equivalencia)::bigint
           end                                        as v_parada_dia
      from com_regra c
    ...
    alter table tmp_calc add column v_planejada  bigint;
    alter table tmp_calc add column v_disponivel bigint;

    update tmp_calc
       set v_planejada = greatest(0, v_parada_dia - min_parada_planejada);
    update tmp_calc
       set v_disponivel = round(v_planejada * oee_pct)::bigint;
    ...
    select v_execucao_id, recurso_id, data, turno_id, ordem, etapa,
      ...
      from (
        select recurso_id, data, turno_id,
               ...
               0::bigint as antes, v_bruto as depois,
