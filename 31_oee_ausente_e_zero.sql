-- =============================================================================
-- 31 — OEE NAO CADASTRADO PASSA A VALER ZERO
--
-- O motor usava coalesce(oee.oee_pct, 1.0): recurso sem OEE cadastrado
-- rendia 100%, e a disponivel saia igual a planejada. O numero era plausivel, e
-- e por isso que era perigoso — a Tecelagem inteira, 173 recursos, estava
-- calculada com OEE efetivo de 100,0% enquanto Beneficiamento marcava 72,3% e
-- Confeccao 80,0%. Ninguem perguntou nada, porque o numero nao tinha cara de
-- errado.
--
-- Agora a ausencia vale ZERO. Nao porque zero seja a verdade — a maquina roda,
-- so nao se sabe com que rendimento —, mas porque zero GRITA e cem sussurra. A
-- decisao e do Bruno, e ela vem junto com a outra metade, sem a qual seria so
-- trocar um erro por outro:
--
--   RECURSO NOVO NASCE COM OEE 100% CADASTRADO.
--
-- E essa metade que faz a regra fechar. Se todo recurso tem OEE desde o
-- nascimento, "sem OEE" deixa de ser o estado comum e vira o que sempre devia
-- ter sido: uma anomalia, que aparece como zero e manda alguem olhar.
--
-- ORDEM: rode este arquivo DEPOIS de garantir que todo recurso tem OEE — o
-- proprio deploy faz isso para os novos, e os 173 existentes foram preenchidos
-- com 100% junto com esta migracao. Rodar antes zera a Tecelagem no proximo
-- Recalcular.
--
-- Recalcular tudo e necessario para o numero mudar: a rodada guardada continua
-- com o oee_pct que valia quando ela rodou.
-- =============================================================================
